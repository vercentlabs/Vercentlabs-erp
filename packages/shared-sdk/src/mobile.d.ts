...
export type MobileClient = {
  ...
  setWorkspaceContext(input: { companyId: string; branchId: string | null }): Promise<{
    message: string;
    session: MobileSession;
    shell: {
      organizations: Array<{ id: string; name: string }>;
  ...
  getCrm(resource: string, id: string): Promise<{ record: Record<string, unknown>; related?: Record<string, Array<Record<string, unknown>>> }>;
  updateCrm(resource: string, id: string, input: Record<string, unknown>, idempotencyKey?: string): Promise<{ record: Record<string, unknown>; message: string }>;
  createCrm(resource: string, input: Record<string, unknown>, idempotencyKey?: string): Promise<{ record: Record<string, unknown>; message: string }>;
  ...
  search(query: string): Promise<{ results: Array<{ resource: string; record: Record<string, unknown>; title: string; subtitle: string; href: string }> }>;
  notifications(): Promise<{ notifications: Array<{ id: string; title: string; message: string; href: string | null; readAt: string | null; createdAt: string }> }>;
  markNotifications(input: { id?: string; all?: boolean }): Promise<{ message: string }>;
...
export type MobileResourceDefinition = {
  ...
  columns?: Array<{ key: string; label: string; optionsKey?: string; format?: string }>;
};
...
export type MobileWorkspaceCatalog = {
  ...
  modules: Array<{ key: string; name: string; description: string; availability: string }>;
};
