...
| FLOW-001 (`ACTIVE <-> INACTIVE -> ARCHIVED`) | **PASS (fixed 2026-09-05).** `reactivateCrmContact` (mirrors `archiveCrmContact`'s idempotent shape) flips `status` back to `active` and clears `archived_at`; it additionally requires the linked Account (if any) to still be active, refusing with `CRM_CONTACT_ACCOUNT_ARCHIVED` otherwise, so reactivation can't produce a contact pointing at an archived account. Wired through the same `PATCH /api/crm/contacts/[id]` route via `{action:"reactivate"}` (the same convention already used by F004's lead-source reactivate route), with a "Reactivate" button on the Contact detail page when `canManage && status !== "active"`. Proven by 3 new backend tests (restore, idempotent no-op, blocked-by-archived-account) and 1 new web wiring test. |
...
Both real gaps found in the initial trace are now fixed: sensitive-field redaction for contact email/phone/mobile (matching Leads' existing pattern), and a governed reactivation path mirroring `archiveCrmContact`. 23 of 37 rows now PASS with cited evidence.
