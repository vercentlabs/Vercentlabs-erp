...
Maintain the authoritative inventory item profile and inventory-control attributes without duplicating commercial product truth.
...
- `STOCK-P4-BE-001` — official primary benchmark evidence in `BENCHMARK_REGISTER.csv`.
- `STOCK-P4-BE-002` — official primary benchmark evidence in `BENCHMARK_REGISTER.csv`.
...
Independent omission review covered **inventory flags, lifecycle, item/company ownership, stocking policy, costing/tracking configuration and immutable historical snapshots**. These expectations are requirements or explicit boundaries; none are silently omitted because the canonical title is short.

## [SPEC-SUBCAPABILITIES] Sub-capabilities and capability mapping
- `F097-CAP-001` — The Item master capability MUST maintain the authoritative inventory item profile and inventory-control attributes without duplicating commercial product truth.
- `F097-CAP-002` — The capability MUST explicitly cover inventory flags, lifecycle, item/company ownership, stocking policy, costing/tracking configuration and immutable historical snapshots; the canonical title is a traceability anchor, not complete scope.
- `F097-CAP-003` — Item master MUST preserve Stock ownership of physical quantity/stock-ledger effects and use public contracts for Procurement, Sales, Manufacturing, Quality, POS, Projects, Assets and Accounting effects.

## [SPEC-FUNCTIONAL] Functional requirements and user stories
- `F097-FR-001` — The system MUST provide a server-backed Item master workflow covering success, validation, permission, conflict, downstream failure, reversal/recovery and audit states.
- `F097-FR-002` — Every material Item master mutation MUST preserve actor, effective/posting time, reason, source reference, prior/new state or immutable event, and downstream lineage.
- `F097-FR-003` — Item master MUST remain usable at enterprise item/location/movement volumes through stable pagination, indexing, batching, virtualization or background processing as appropriate.
- `F097-US-001` — As an authorized warehouse/inventory operator, I can perform Item master accurately without bypassing quantity, traceability, quality, costing or permission controls.
- `F097-US-002` — As an inventory manager/controller, I can review exceptions, history, reconciliation and policy for Item master within my permitted company/warehouse scope.

## [SPEC-FLOWS] Primary, alternate, exception, retry and reversal flows
- `F097-FLOW-001` — The governed lifecycle MUST follow ACTIVE/VALID -> versioned change or controlled inactivation; posted historical facts remain immutable, with explicit guards and no silent history rewrite.
- `F097-FLOW-002` — Validation, permission, concurrency and downstream failures for Item master MUST be actionable and safely retryable without duplicate physical, reservation, valuation or accounting effects.
...
- Aggregate owner: Stock domain service for **Item master**.
...
- `F097-DATA-001` — The Item master model MUST define organization/company, item, warehouse/location and lot/batch/serial dimensions as applicable, with stable keys, constraints, indexes, retention and lineage.
- `F097-DATA-002` — Quantity, UOM, cost, status, source and policy values that determine historical Item master interpretation MUST remain reproducible after master/configuration changes.

## [SPEC-VALIDATION] Validation rules
- `F097-VAL-001` — All Item master commands MUST validate tenant/company scope, item/location eligibility, UOM, quantity sign/precision, tracking/status rules, lifecycle and reference integrity server-side.
- `F097-VAL-002` — Item master failures MUST use stable domain error codes and corrective guidance without exposing unauthorized cost, supplier, customer or cross-company data.

## [SPEC-BUSINESS-RULES] Business rules and invariants
- `F097-BR-001` — The Stock domain command responsible for Item master is authoritative; UI, API, import, barcode, automation and AI paths MUST reuse the same invariant checks.
- `F097-BR-002` — Posted stock facts for Item master MUST be corrected by reversal/compensation or linked adjustment, never by rewriting authoritative movement history.

## [SPEC-CALCULATIONS] Calculations, precision and rounding
- `F097-CALC-001` — All authoritative quantity, availability, ATP, conversion, valuation, aging or replenishment calculations applicable to Item master MUST use deterministic decimal/date rules with explicit precision and reproducible inputs; AI is never authoritative.

## [SPEC-VIEWS] Required view archetypes
- `F097-UX-001` — The Item master workspace MUST expose item/location identity, state, quantity/cost facts as permitted, source/next action, exceptions, related records and history in one coherent Stock shell.
- `F097-UX-002` — Desktop/tablet/phone views for Item master MUST define loading, empty, validation, permission, stale/conflict, offline/retry, destructive confirmation and success states.
- `F097-UX-003` — Warehouse-heavy Item master interactions MUST support keyboard operation, touch-safe controls, scanner/manual alternatives, visible focus and non-drag alternatives.
...
- `F097-AUTO-001` — Automation MAY create proposals/work for Item master only through idempotent Stock/public commands; no automation may directly mutate stock balance tables.

## [SPEC-APPROVALS] Approvals, maker-checker and segregation of duties
- `F097-APP-001` — Where policy requires maker-checker for high-value adjustments, negative-stock exceptions, count variances or quality release affecting Item master, approval MUST bind to the exact version/value being approved.

## [SPEC-NOTIFICATIONS] Notifications and communication behavior
- `F097-NOTIF-001` — Notifications for Item master MUST be event-driven, deduplicated, preference/permission aware and deep-link to the exact exception or work item without leaking restricted data.
...
- `F097-REP-001` — Reports/KPIs for Item master MUST define grain, filters, freshness, drilldown, as-of semantics and reconciliation to authoritative movement/balance/valuation records.

## [SPEC-AI] AI opportunities, authority boundary and safeguards
- `F097-AI-001` — AI for Item master is limited to assist/recommend/generate workflows with provenance, explanation and permission filtering; it MUST NOT be authoritative for stock quantity, reservation, valuation, traceability, quality holds or posting legality.

## [SPEC-SECURITY] Security, permissions and field controls
- `F097-SEC-001` — Every Item master query and mutation MUST enforce authentication, module entitlement, organization/company/warehouse scope, action permission and record/dimension visibility server-side.
- `F097-SEC-002` — Cost/valuation, customer/supplier references and administrative configuration in Item master MUST be field/action restricted and protected against IDOR, aggregate leakage and unauthorized export.
...
- `F097-INT-001` — Inbound cross-module requests affecting Item master MUST call a versioned public Stock command with authorization, idempotency key, source reference, validation and explicit failure result.
- `F097-INT-002` — Outbound effects from Item master MUST use outbox/public events or orchestration with retry, reversal/compensation and reconciliation; Stock MUST NOT write another module’s private tables.

## [SPEC-API] Commands, queries and API contracts
- `F097-API-001` — Item master mutation APIs MUST define request schema, authorization, idempotency, expected state/version, stable errors, audit/outbox effects and compatibility semantics.
- `F097-API-002` — Item master query APIs MUST define dimension grain, pagination/filter/sort, as-of/freshness semantics, authorization-safe aggregates and stable response contracts.
...
- `F097-PERF-001` — Item master MUST define performance envelopes for 0/1/100/10k/1m movement or dimension records as applicable, with bounded queries and explicit asynchronous thresholds.

## [SPEC-OBSERVABILITY] Logs, metrics, traces, jobs and support diagnostics
- `F097-OBS-001` — Item master MUST emit structured logs/metrics/traces for failures, retries, lock contention, drift/reconciliation, job lag and downstream delivery with correlation IDs.
...
- Verified current-code evidence: `STOCK-P4-CODE-001` → `database/tenant/migrations/001_business_data_foundation.sql`.
...
- `F097-E2E-001` — E2E MUST prove an authorized operator can complete the primary Item master journey and observe correct quantity/state/history/downstream references.
- `F097-E2E-002` — E2E MUST cover unauthorized access, invalid/stale data, concurrent/replayed mutation and one relevant failure, short/partial, hold, reversal or reconciliation path for Item master.

## [SPEC-UAT] Human UAT plan
- `F097-UAT-001` — A real warehouse/inventory operator MUST execute the primary Item master job with realistic data on desktop and an appropriate mobile/scanner path, capturing visible and data evidence.
- `F097-UAT-002` — An inventory manager/controller plus relevant Quality/Finance/Sales/Procurement persona MUST verify permissions, exception/reversal behavior, audit and cross-module reconciliation for Item master.
...
- Required flow IDs: `F097-PFC-01`, `F097-PFC-02`, `F097-PFC-03`, `F097-PFC-04`, `F097-PFC-05`, `F097-PFC-06`, `F097-PFC-07`, `F097-PFC-08`, `F097-PFC-09`, `F097-PFC-10`
- State transition IDs: `F097-STM-01`, `F097-STM-02`, `F097-STM-03`, `F097-STM-04`, `F097-STM-05`
...
- Curated authoritative benchmark IDs: `PFD-BM-F097-1`; `PFD-BM-F097-2`
- The Pass D mappings are the implementation-planning benchmark authority for **Item master**.
