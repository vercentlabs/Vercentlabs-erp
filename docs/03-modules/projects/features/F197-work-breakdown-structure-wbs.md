...
What would an experienced enterprise operator reasonably expect from **Work Breakdown Structure (WBS)** that the short canonical name does not explicitly state? Every material omission must be decided, not silently ignored.
...
Use traceable IDs such as `F197-FR-001`. TBD.

## [SPEC-BUSINESS-RULES] Business rules and calculations
Use `F197-BR-###`; define invariants, formulas, precision, rounding, currency/UOM/timezone behavior and effective dating where applicable.
...
Use `F197-DATA-###`; define fields, keys, relationships, constraints, indexes, lifecycle retention/archive, precision, localization and migration implications.
...
Use `F197-UX-###`; define create/edit/detail/quick-create/related-record flows and loading, empty, validation, error, conflict, permission and success states.
...
Use `F197-AI-###`; define provenance, confidence, permission boundary, human oversight, deterministic authority boundary, fallback, feedback and audit.

## [SPEC-SECURITY] Security, privacy and field/record scope
Use `F197-SEC-###`; define authentication assumptions, organization/company/branch/location/record scope, field controls, sensitive data handling, abuse cases and negative authorization tests.
...
Use `F197-OBS-###`; define transaction boundaries, optimistic/pessimistic concurrency, retry/idempotency keys, outbox/worker behavior, duplicate prevention, metrics/logs/traces, exception queues and reconciliation.

## [SPEC-INTEGRATIONS] Cross-module and external integrations
Use `F197-INT-###`; for each transition define trigger, source owner, destination public command, validation, transaction boundary, idempotency, retry/failure, audit/event, visible state, reversal and reconciliation.
...
Use `F197-E2E-###` for end-to-end requirements; include unit/domain, database/security, API/integration, browser E2E, cross-browser/device, performance/scale and failure/recovery verification.

## [SPEC-UAT] Human UAT plan
Use `F197-UAT-###`; define operator scenario, prerequisites, steps, expected state/data/audit outcomes, visual evidence and sign-off.
