...
Provide project lifecycle/state machine with controlled draft/planned/active/on-hold/completed/closed/cancelled transitions and close/reopen reconciliation.
...
- `PRJ-P6-BE-023` — official benchmark evidence in `BENCHMARK_REGISTER.csv`.
- `PRJ-P6-BE-024` — official benchmark evidence in `BENCHMARK_REGISTER.csv`.
...
Independent omission review covered hierarchy/dependency cycles, baselines, calendars, resource overallocation, approvals, locks/reopen, commitments/actuals, billing duplication, FX/tax handoff, project close/reopen, audit, responsive/mobile UX, collaboration visibility, reconciliation and scale behavior expected for **Project status**. No material expectation is silently omitted because the canonical title is short.

## [SPEC-SUBCAPABILITIES] Sub-capabilities and capability mapping
- `F204-CAP-001` — Project status MUST provide project lifecycle/state machine with controlled draft/planned/active/on-hold/completed/closed/cancelled transitions and close/reopen reconciliation.
- `F204-CAP-002` — Project status MUST cover lifecycle, permissions, approvals, schedule/baseline semantics, concurrency, retry/reversal, audit, reporting, responsive UX and scale; the canonical title is a traceability anchor, not the whole capability.
- `F204-CAP-003` — Project status MUST preserve Projects ownership of project delivery state while Sales, Procurement, Stock, HR/Payroll and Accounting effects use public contracts rather than private-table mutation.

## [SPEC-FUNCTIONAL] Functional requirements and user stories
- `F204-FR-001` — The system MUST provide a server-backed Project status workflow covering success, validation, permission, dependency/capacity, billing/period, conflict, downstream failure, retry/reversal and audit states.
- `F204-FR-002` — Every material Project status action MUST preserve actor, effective/posting time, reason, source project/WBS/task/baseline/contract, prior/new state or immutable event, and downstream lineage.
- `F204-FR-003` — Project status MUST remain usable at enterprise project/task/time/financial volumes using bounded queries, pagination/virtualization, async jobs and stable transitions where appropriate.
- `F204-US-001` — As an authorized project manager/controller/team member, I can perform Project status with the exact work, schedule, resource, customer and financial context needed for my role.
- `F204-US-002` — As a PMO/resource/finance approver, I can review exceptions, provenance, approvals, variances, history and reconciliation for Project status within permitted company/project/team scope.

## [SPEC-FLOWS] Primary, alternate, exception, retry and reversal flows
- `F204-FLOW-001` — The governed lifecycle for Project status MUST follow ACTIVE/VALID -> controlled update/closure; historical delivery and financial facts remain reproducible with explicit guards and no silent historical rewrite.
- `F204-FLOW-002` — Validation, permission, dependency/capacity, concurrent update, billing eligibility, period lock and downstream failures for Project status MUST be actionable and safely retryable without duplicate cost, time, stock, procurement or invoice effects.
...
- Aggregate owner: Projects domain service for **Project status**.
...
- `F204-DATA-001` — The Project status model MUST define organization/company/branch, project/customer/order, WBS/task/milestone/resource, currency/rate/budget/billing and source-document dimensions as applicable, with stable keys, constraints, indexes, retention and lineage.
- `F204-DATA-002` — Dates, durations, effort, quantity/UOM, currency/FX, cost/bill rates, baseline/version, approval and source values that determine historical Project status interpretation MUST remain reproducible after master/configuration changes.

## [SPEC-VALIDATION] Validation rules
- `F204-VAL-001` — All Project status commands MUST validate tenant/company/project scope, lifecycle, date/range/precision, hierarchy/dependency integrity, resource/financial eligibility and cross-reference integrity server-side.
- `F204-VAL-002` — Project status failures MUST expose stable domain error codes and corrective guidance without leaking unauthorized customer, employee, cost/rate, margin, document or cross-project data.

## [SPEC-BUSINESS-RULES] Business rules and invariants
- `F204-BR-001` — The Projects domain command responsible for Project status is authoritative for project delivery rules; UI, API, import, automation and AI paths MUST reuse the same deterministic invariant checks.
- `F204-BR-002` — Project status MUST preserve approved baselines, source links and snapshotted rates/eligibility inputs needed to reproduce historical schedule, cost, revenue and billing outcomes after configuration changes.

## [SPEC-CALCULATIONS] Calculations, precision and rounding
- `F204-CALC-001` — All scheduling, allocation, roll-up, progress, budget/cost/revenue, billing, FX, margin, ETC/EAC/VAC or variance calculations applicable to Project status MUST use deterministic decimal/date/calendar rules, explicit rounding and reproducible inputs; AI is never authoritative.

## [SPEC-VIEWS] Required view archetypes
- `F204-UX-001` — The Project status workspace MUST expose identity, lifecycle, WBS/task/resource/customer context, schedule, financial facts as permitted, exceptions, related records and history in one coherent Projects shell.
- `F204-UX-002` — Desktop/tablet/phone views for Project status MUST define loading, empty, validation, permission, dependency/overallocation, stale/conflict, offline/retry, destructive confirmation and success states.
- `F204-UX-003` — Planning-heavy Project status interactions MUST support keyboard/touch operation, non-drag alternatives, visible focus, non-color status cues, screen-reader semantics and dense-view alternatives.
...
- `F204-AUTO-001` — Automation MAY create proposals/work for Project status only through idempotent Projects/public commands; it MUST NOT directly mutate source ledgers, stock balances, payroll, customer invoices or accounting entries.

## [SPEC-APPROVALS] Approvals, maker-checker and segregation of duties
- `F204-APP-001` — Where policy requires approval for budgets, time/expenses, billing, project closure/reopen, high-risk changes or Project status, approval MUST bind to the exact version/state/value and prohibit self-approval where configured.

## [SPEC-NOTIFICATIONS] Notifications and communication behavior
- `F204-NOTIF-001` — Notifications for Project status MUST be event-driven, deduplicated, preference/permission aware and deep-link to the exact task, approval, risk, overrun or billing item without exposing restricted data.
...
- `F204-REP-001` — Reports/KPIs for Project status MUST define grain, filters, baseline/as-of/freshness semantics, drilldown and reconciliation to authoritative project, Sales, Procurement, Stock, HR and Accounting records.

## [SPEC-AI] AI opportunities, authority boundary and safeguards
- `F204-AI-001` — AI for Project status is limited to assist/recommend/generate workflows with provenance, freshness, uncertainty, explanation and permission filtering; AI MUST NOT be authoritative for authorization, schedule legality, approval, billable eligibility, project accounting, invoice or source-ledger truth.

## [SPEC-SECURITY] Security, permissions and field controls
- `F204-SEC-001` — Every Project status query and mutation MUST enforce authentication, module entitlement, organization/company/project/team/record scope, action permission and server-side field visibility.
- `F204-SEC-002` — Customer contracts, employee allocations/time, costs/rates/margin, approvals and restricted documents/comments in Project status MUST be field/action restricted and protected against IDOR, aggregate leakage and unauthorized export.
...
- `F204-INT-001` — Inbound cross-module requests affecting Project status MUST call a versioned Projects public command with authorization, idempotency/source reference, validation and explicit success/failure result.
- `F204-INT-002` — Outbound effects from Project status MUST use public commands/events/orchestration with retry, reversal/compensation and reconciliation; Projects MUST NOT mutate another module’s private tables.

## [SPEC-API] Commands, queries and API contracts
- `F204-API-001` — Project status mutation APIs MUST define request schema, authorization, idempotency, expected state/version, stable errors, audit/outbox effects and compatibility semantics.
- `F204-API-002` — Project status query APIs MUST define project/WBS/resource/financial grain, pagination/filter/sort, as-of/baseline semantics, authorization-safe aggregates and stable responses.
...
- `F204-PERF-001` — Project status MUST define performance envelopes for 0/1/100/10k/1m project tasks, time entries, cost lines or reporting facts as applicable, with bounded synchronous work and explicit async thresholds.

## [SPEC-OBSERVABILITY] Logs, metrics, traces, jobs and support diagnostics
- `F204-OBS-001` — Project status MUST emit structured logs/metrics/traces for failures, retries, lock contention, schedule recalculation/job lag, approval/billing drift and reconciliation using correlation IDs.
...
- `PRJ-P6-CODE-012` — verified current-code evidence: `services/api/src/modules/projects/index.js`.
...
- `F204-E2E-001` — E2E MUST prove an authorized persona can complete the primary Project status journey and observe correct schedule/resource/state/history and downstream effects.
- `F204-E2E-002` — E2E MUST cover unauthorized access, stale/concurrent/replayed action and at least one relevant dependency, allocation conflict, rejection, partial billing, reversal, close/reopen or reconciliation path for Project status.

## [SPEC-UAT] Human UAT plan
- `F204-UAT-001` — A real project manager/controller/team member MUST execute the primary Project status job with realistic data on appropriate desktop/tablet/mobile form factors, capturing visible and data evidence.
- `F204-UAT-002` — A PMO/resource manager plus relevant Sales/Procurement/Stock/HR/Finance persona MUST verify permissions, exceptions, reversal/reconciliation, audit and cross-module outcomes for Project status.

## [SPEC-DOD] Objective Definition of Done
**Project status is done only when** all approved requirements are implemented through normal authorized commands, required responsive/accessibility states exist, audit/retry/reversal/reconciliation behavior is proven, automated/E2E/UAT evidence passes and parent capability/journey gates pass. A table/API/page alone is insufficient.
...
- Required flow IDs: `F204-PFC-01`, `F204-PFC-02`, `F204-PFC-03`, `F204-PFC-04`, `F204-PFC-05`, `F204-PFC-06`, `F204-PFC-07`, `F204-PFC-08`, `F204-PFC-09`, `F204-PFC-10`
- State transition IDs: `F204-STM-01`, `F204-STM-02`, `F204-STM-03`, `F204-STM-04`, `F204-STM-05`
...
- Curated authoritative benchmark IDs: `PFD-BM-F204-1`; `PFD-BM-F204-2`
- The Pass D mappings are the implementation-planning benchmark authority for **Project status**.
