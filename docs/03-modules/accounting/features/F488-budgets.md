...
What would an experienced enterprise operator reasonably expect from **Budgets** that the short canonical name does not explicitly state? Every material omission must be decided, not silently ignored.
...
Use traceable IDs such as `F488-FR-001`. TBD.

## [SPEC-BUSINESS-RULES] Business rules and calculations
Use `F488-BR-###`; define invariants, formulas, precision, rounding, currency/UOM/timezone behavior and effective dating where applicable.
...
Use `F488-DATA-###`; define fields, keys, relationships, constraints, indexes, lifecycle retention/archive, precision, localization and migration implications.
...
Use `F488-UX-###`; define create/edit/detail/quick-create/related-record flows and loading, empty, validation, error, conflict, permission and success states.
...
Use `F488-AI-###`; define provenance, confidence, permission boundary, human oversight, deterministic authority boundary, fallback, feedback and audit.

## [SPEC-SECURITY] Security, privacy and field/record scope
Use `F488-SEC-###`; define authentication assumptions, organization/company/branch/location/record scope, field controls, sensitive data handling, abuse cases and negative authorization tests.
...
Use `F488-OBS-###`; define transaction boundaries, optimistic/pessimistic concurrency, retry/idempotency keys, outbox/worker behavior, duplicate prevention, metrics/logs/traces, exception queues and reconciliation.

## [SPEC-INTEGRATIONS] Cross-module and external integrations
Use `F488-INT-###`; for each transition define trigger, source owner, destination public command, validation, transaction boundary, idempotency, retry/failure, audit/event, visible state, reversal and reconciliation.
...
Use `F488-E2E-###` for end-to-end requirements; include unit/domain, database/security, API/integration, browser E2E, cross-browser/device, performance/scale and failure/recovery verification.

## [SPEC-UAT] Human UAT plan
Use `F488-UAT-###`; define operator scenario, prerequisites, steps, expected state/data/audit outcomes, visual evidence and sign-off.
