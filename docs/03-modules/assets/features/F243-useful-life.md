...
Provide enterprise-grade useful life with lifecycle, exception, audit, security, reconciliation and operator UX controls.
...
- `AST-P7-BE-025` — official benchmark evidence in `BENCHMARK_REGISTER.csv`.
- `AST-P7-BE-026` — official benchmark evidence in `BENCHMARK_REGISTER.csv`.
...
Independent omission review for **Useful life** covered identity uniqueness, hierarchy/component/multi-book implications where relevant, effective dating, thresholds/CIP/partial-transfer/disposal expectations, depreciation convention/rounding, period close, warranty/maintenance/calibration, field/offline evidence, SoD, concurrency, idempotency, reversal/reconciliation, accessibility and scale. No material enterprise expectation is silently omitted because the canonical title is short.

## [SPEC-SUBCAPABILITIES] Sub-capabilities and capability mapping
- `F243-CAP-001` — Useful life MUST provide enterprise-grade useful life with lifecycle, exception, audit, security, reconciliation and operator UX controls.
- `F243-CAP-002` — Useful life MUST define lifecycle/state, effective dating, authorization, approvals, corrections/reversal and immutable history where facts are financial or compliance relevant.
- `F243-CAP-003` — Useful life MUST expose auditable public contracts/events and evidence needed for Procurement, Stock, Projects, Manufacturing, HR and Accounting interactions without direct private-table mutation.

## [SPEC-FUNCTIONAL] Functional requirements and user stories
- `F243-FR-001` — Authorized users MUST be able to execute the primary Useful life lifecycle with actionable validation, explicit state and durable audit evidence.
- `F243-FR-002` — Users MUST be able to search, filter, inspect exceptions/history/related records for Useful life and resolve eligible exceptions without bypassing controls.
- `F243-FR-003` — Approved or posted Useful life facts MUST be corrected through controlled amendment/reversal/reopen mechanisms rather than silent destructive edits.
- `F243-US-001` — As an asset operator, I can perform Useful life from a focused workspace and understand status, ownership, next action and downstream effect.
- `F243-US-002` — As a controller/auditor, I can prove who changed or approved Useful life, why, under which effective date/book/company, and how it reconciles downstream.

## [SPEC-FLOWS] Primary, alternate, exception, retry and reversal flows
- `F243-FLOW-001` — The primary Useful life flow MUST validate prerequisites -> authorize -> lock/version relevant records -> apply one atomic domain transition -> audit -> emit idempotent downstream intent -> show outcome.
- `F243-FLOW-002` — Useful life MUST cover invalid source, stale version, duplicate request, permission denial, closed period, partial downstream failure, retry, cancellation/reversal and reconciliation without duplicated business effect.
...
- Aggregate owner: Assets domain service for **Useful life**.
...
- `F243-DATA-001` — Useful life MUST persist stable identifiers, organization/company scope, asset/category/source references, status, effective dates, actor/version and feature-specific values required for enterprise-grade useful life with lifecycle, exception, audit, security, reconciliation and operator UX controls.
- `F243-DATA-002` — Useful life MUST retain lineage to source procurement/accounting/maintenance/inspection/disposal records, attachments/evidence, approvals, reversals and correlation/idempotency identifiers where applicable.

## [SPEC-VALIDATION] Validation rules
- `F243-VAL-001` — Useful life MUST reject malformed identifiers, invalid dates/amounts/units, inactive references, cross-company references, duplicate identities and impossible lifecycle combinations with actionable errors.
- `F243-VAL-002` — Useful life MUST validate current asset state, open assignments/maintenance/verification obligations, book/period status and downstream eligibility before committing a controlled transition.

## [SPEC-BUSINESS-RULES] Business rules and invariants
- `F243-BR-001` — Useful life MUST obey authoritative asset lifecycle, company/book/effective-date, valuation, custody/maintenance and period rules; AI or UI state cannot override these invariants.
- `F243-BR-002` — Once Useful life creates an approved, posted, completed or compliance-significant fact, history MUST remain immutable and corrections MUST link to the original fact.

## [SPEC-CALCULATIONS] Calculations, precision and rounding
- `F243-CALC-001` — Any numeric/date result for Useful life MUST define units, currency/precision where relevant, timezone/effective-date boundaries, rounding and reproducibility; no AI-generated value may become authoritative.

## [SPEC-VIEWS] Required view archetypes
- `F243-UX-001` — Desktop UX for Useful life MUST expose identity/status, key facts, exception banners, related records, audit and eligible actions without forcing operators through generic CRUD screens.
- `F243-UX-002` — Mobile/tablet UX for Useful life MUST support field-relevant scan/search, large touch targets, evidence/photo/document capture where applicable, explicit offline boundary and conflict-safe sync.
- `F243-UX-003` — Useful life MUST meet WCAG 2.2 AA intent with semantic labels, keyboard/focus order, announced errors/status, non-color-only state, target sizing and non-drag alternatives.
...
- `F243-AUTO-001` — Background automation for Useful life MAY generate due work, schedules, alerts, draft calculations or integration jobs, but MUST use normal domain authorization/system policy, idempotency, audit and reconciliation controls.

## [SPEC-APPROVALS] Approvals, maker-checker and segregation of duties
- `F243-APP-001` — Approval-required Useful life transitions MUST snapshot material evidence/values, record approver/reason/time, block prohibited self-approval and invalidate/re-review when material inputs change.

## [SPEC-NOTIFICATIONS] Notifications and communication behavior
- `F243-NOTIF-001` — Useful life notifications MUST be event-driven, deduplicated, permission-safe and actionable for due/overdue, exception, approval, expiry/warranty, maintenance/calibration or reconciliation conditions as applicable.
...
- `F243-REP-001` — Useful life reporting MUST define metric formulas, as-of/effective-date/book/company scope, permission-safe drilldown, export and reconciliation to authoritative source facts.

## [SPEC-AI] AI opportunities, authority boundary and safeguards
- `F243-AI-001` — AI for Useful life MAY extract documents, classify/analyze anomalies, explain schedules or recommend maintenance/verification priorities, but MUST show provenance/uncertainty and MUST NOT authoritatively determine ledger, depreciation, tax, authorization or legal state transitions.

## [SPEC-SECURITY] Security, permissions and field controls
- `F243-SEC-001` — Every Useful life query/command MUST enforce server-side organization, company/branch/location/record scope plus action permission; unauthorized totals, finance fields, custodian data and documents MUST not leak.
- `F243-SEC-002` — Sensitive Useful life actions MUST enforce maker-checker/SoD policy where configured; creators/requesters/technicians cannot self-approve capitalization, financial posting, impairment/revaluation, disposal or compliance closure when prohibited.
...
- `F243-INT-001` — Useful life MUST consume external-module facts only through versioned public contracts with source ID, company, effective date, payload validation, authorization/system trust, retry semantics and reconciliation.
- `F243-INT-002` — Useful life MUST publish downstream intent/event with stable source identity and idempotency key; Procurement, Stock, Manufacturing, Projects, HR or Accounting retain ownership of their private state and may reject/reconcile independently.

## [SPEC-API] Commands, queries and API contracts
- `F243-API-001` — Useful life commands MUST define request schema, permission/scope, expected state/version, idempotency where externally visible, domain errors, transaction boundary, audit and downstream effects.
- `F243-API-002` — Useful life queries MUST define pagination/filter/sort, as-of/effective-date semantics where relevant, permission-safe aggregates, stable identifiers and compatibility/versioning behavior.
...
- `F243-PERF-001` — Useful life MUST define behavior for 0/1/100/10k/1m relevant rows, server pagination/virtualization, batch/background thresholds and latency budgets so enterprise asset registers/history do not degrade into unbounded scans.

## [SPEC-OBSERVABILITY] Logs, metrics, traces, jobs and support diagnostics
- `F243-OBS-001` — Useful life MUST emit structured logs/metrics/traces with correlation IDs, job/retry/dead-letter state, business exception counters and reconciliation diagnostics without exposing sensitive financial/custodian data.
...
- `AST-P7-CODE-013` — verified current-code evidence: `services/api/src/modules/accounting/assets.js`.
...
- `F243-E2E-001` — Browser/device E2E MUST prove the primary authorized Useful life journey including visible state, persisted data, audit and downstream contract outcome.
- `F243-E2E-002` — E2E MUST prove Useful life permission denial, stale/conflict handling, duplicate/retry behavior and reversal/reconciliation or exception recovery without duplicated effect.

## [SPEC-UAT] Human UAT plan
- `F243-UAT-001` — A realistic asset operator MUST execute Useful life using documented prerequisites/steps and verify visible, data, audit and downstream outcomes with evidence capture.
- `F243-UAT-002` — A controller/auditor MUST independently verify Useful life authorization/SoD, history, calculation/reconciliation and exception/reversal behavior before sign-off.
...
- `F243-SEM-01` — **Asset lifecycle from acquisition through disposal**: Define acquired/CIP/capitalized/in-service/held/disposed states, transfers/assignments and effective-dated ownership/custody.
- `F243-SEM-02` — **Asset identity, book/value/location/custody/maintenance history**: Freeze identifiers, category, cost/value, useful life, salvage, books, location/custodian, maintenance and source lineage.
- `F243-SEM-03` — **Capitalization/depreciation/revaluation/maintenance/disposal rules**: Define thresholds/effectivity, depreciation methods/conventions, impairment/revaluation, maintenance/calibration and gain/loss behavior.
- `F243-SEM-04` — **Custody/finance/maintenance authority and SoD**: Separate create/capitalize/transfer/revalue/dispose/post authority and protect financial/sensitive asset data.
- `F243-SEM-05` — **Register, custody, maintenance, verification and disposal workspace**: Cover barcode/QR, field verification, transfer/assignment, maintenance/inspection, financial book view, mobile and accessibility.
- `F243-SEM-06` — **Procurement/hr/projects/accounting contracts**: Create from procurement, link employee/project custody, post depreciation/disposal via public finance contracts and reconcile.
- `F243-SEM-07` — **Partial transfer/disposal, backdating, book lock and missing asset recovery**: Handle concurrent custody changes, retro changes, locked periods, duplicate posting, lost/damaged assets and reversals.
- `F243-SEM-08` — **Depreciation/book/custody/reconciliation verification**: Require calculation goldens, effective-date tests, barcode verification, accounting reconciliation, E2E and UAT.
...
- Required flow IDs: `F243-PFC-01`, `F243-PFC-02`, `F243-PFC-03`, `F243-PFC-04`, `F243-PFC-05`, `F243-PFC-06`, `F243-PFC-07`, `F243-PFC-08`, `F243-PFC-09`, `F243-PFC-10`
- State transition IDs: `F243-STM-01`, `F243-STM-02`, `F243-STM-03`, `F243-STM-04`, `F243-STM-05`
...
- Curated authoritative benchmark IDs: `PFD-BM-F243-1`; `PFD-BM-F243-2`
- The Pass D mappings are the implementation-planning benchmark authority for **Useful life**.
