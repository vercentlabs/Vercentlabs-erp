# F245 — Straight-line depreciation

## [SPEC-IDENTITY] Identity and traceability
- Canonical ID: `F245`
- Canonical name: **Straight-line depreciation**
- Module: **Assets**
...
Provide straight-line depreciation with explicit depreciable base, convention, period allocation, rounding and final true-up.
...
- `AST-P7-BE-029` — official benchmark evidence in `BENCHMARK_REGISTER.csv`.
- `AST-P7-BE-030` — official benchmark evidence in `BENCHMARK_REGISTER.csv`.
...
Independent omission review for **Straight-line depreciation** covered identity uniqueness, hierarchy/component/multi-book implications where relevant, effective dating, thresholds/CIP/partial-transfer/disposal expectations, depreciation convention/rounding, period close, warranty/maintenance/calibration, field/offline evidence, SoD, concurrency, idempotency, reversal/reconciliation, accessibility and scale. No material enterprise expectation is silently omitted because the canonical title is short.

## [SPEC-SUBCAPABILITIES] Sub-capabilities and capability mapping
- `F245-CAP-001` — Straight-line depreciation MUST provide straight-line depreciation with explicit depreciable base, convention, period allocation, rounding and final true-up.
- `F245-CAP-002` — Straight-line depreciation MUST define lifecycle/state, effective dating, authorization, approvals, corrections/reversal and immutable history where facts are financial or compliance relevant.
- `F245-CAP-003` — Straight-line depreciation MUST expose auditable public contracts/events and evidence needed for Procurement, Stock, Projects, Manufacturing, HR and Accounting interactions without direct private-table mutation.

## [SPEC-FUNCTIONAL] Functional requirements and user stories
- `F245-FR-001` — Authorized users MUST be able to execute the primary Straight-line depreciation lifecycle with actionable validation, explicit state and durable audit evidence.
- `F245-FR-002` — Users MUST be able to search, filter, inspect exceptions/history/related records for Straight-line depreciation and resolve eligible exceptions without bypassing controls.
- `F245-FR-003` — Approved or posted Straight-line depreciation facts MUST be corrected through controlled amendment/reversal/reopen mechanisms rather than silent destructive edits.
- `F245-US-001` — As an asset operator, I can perform Straight-line depreciation from a focused workspace and understand status, ownership, next action and downstream effect.
- `F245-US-002` — As a controller/auditor, I can prove who changed or approved Straight-line depreciation, why, under which effective date/book/company, and how it reconciles downstream.

## [SPEC-FLOWS] Primary, alternate, exception, retry and reversal flows
- `F245-FLOW-001` — The primary Straight-line depreciation flow MUST validate prerequisites -> authorize -> lock/version relevant records -> apply one atomic domain transition -> audit -> emit idempotent downstream intent -> show outcome.
- `F245-FLOW-002` — Straight-line depreciation MUST cover invalid source, stale version, duplicate request, permission denial, closed period, partial downstream failure, retry, cancellation/reversal and reconciliation without duplicated business effect.
...
- Aggregate owner: Assets domain service for **Straight-line depreciation**.
...
- `F245-DATA-001` — Straight-line depreciation MUST persist stable identifiers, organization/company scope, asset/category/source references, status, effective dates, actor/version and feature-specific values required for straight-line depreciation with explicit depreciable base, convention, period allocation, rounding and final true-up.
- `F245-DATA-002` — Straight-line depreciation MUST retain lineage to source procurement/accounting/maintenance/inspection/disposal records, attachments/evidence, approvals, reversals and correlation/idempotency identifiers where applicable.

## [SPEC-VALIDATION] Validation rules
- `F245-VAL-001` — Straight-line depreciation MUST reject malformed identifiers, invalid dates/amounts/units, inactive references, cross-company references, duplicate identities and impossible lifecycle combinations with actionable errors.
- `F245-VAL-002` — Straight-line depreciation MUST validate current asset state, open assignments/maintenance/verification obligations, book/period status and downstream eligibility before committing a controlled transition.

## [SPEC-BUSINESS-RULES] Business rules and invariants
- `F245-BR-001` — Straight-line depreciation MUST obey authoritative asset lifecycle, company/book/effective-date, valuation, custody/maintenance and period rules; AI or UI state cannot override these invariants.
- `F245-BR-002` — Once Straight-line depreciation creates an approved, posted, completed or compliance-significant fact, history MUST remain immutable and corrections MUST link to the original fact.

## [SPEC-CALCULATIONS] Calculations, precision and rounding
- `F245-CALC-001` — Straight-line depreciation MUST use an explicit depreciable base (capitalized cost minus salvage value), useful-life periods and convention; rounding residuals are true-d up in the final eligible period without depreciating below salvage.

## [SPEC-VIEWS] Required view archetypes
- `F245-UX-001` — Desktop UX for Straight-line depreciation MUST expose identity/status, key facts, exception banners, related records, audit and eligible actions without forcing operators through generic CRUD screens.
- `F245-UX-002` — Mobile/tablet UX for Straight-line depreciation MUST support field-relevant scan/search, large touch targets, evidence/photo/document capture where applicable, explicit offline boundary and conflict-safe sync.
- `F245-UX-003` — Straight-line depreciation MUST meet WCAG 2.2 AA intent with semantic labels, keyboard/focus order, announced errors/status, non-color-only state, target sizing and non-drag alternatives.
...
- `F245-AUTO-001` — Background automation for Straight-line depreciation MAY generate due work, schedules, alerts, draft calculations or integration jobs, but MUST use normal domain authorization/system policy, idempotency, audit and reconciliation controls.

## [SPEC-APPROVALS] Approvals, maker-checker and segregation of duties
- `F245-APP-001` — Approval-required Straight-line depreciation transitions MUST snapshot material evidence/values, record approver/reason/time, block prohibited self-approval and invalidate/re-review when material inputs change.

## [SPEC-NOTIFICATIONS] Notifications and communication behavior
- `F245-NOTIF-001` — Straight-line depreciation notifications MUST be event-driven, deduplicated, permission-safe and actionable for due/overdue, exception, approval, expiry/warranty, maintenance/calibration or reconciliation conditions as applicable.
...
- `F245-REP-001` — Straight-line depreciation reporting MUST define metric formulas, as-of/effective-date/book/company scope, permission-safe drilldown, export and reconciliation to authoritative source facts.

## [SPEC-AI] AI opportunities, authority boundary and safeguards
- `F245-AI-001` — AI for Straight-line depreciation MAY extract documents, classify/analyze anomalies, explain schedules or recommend maintenance/verification priorities, but MUST show provenance/uncertainty and MUST NOT authoritatively determine ledger, depreciation, tax, authorization or legal state transitions.

## [SPEC-SECURITY] Security, permissions and field controls
- `F245-SEC-001` — Every Straight-line depreciation query/command MUST enforce server-side organization, company/branch/location/record scope plus action permission; unauthorized totals, finance fields, custodian data and documents MUST not leak.
- `F245-SEC-002` — Sensitive Straight-line depreciation actions MUST enforce maker-checker/SoD policy where configured; creators/requesters/technicians cannot self-approve capitalization, financial posting, impairment/revaluation, disposal or compliance closure when prohibited.
...
- `F245-INT-001` — Straight-line depreciation MUST consume external-module facts only through versioned public contracts with source ID, company, effective date, payload validation, authorization/system trust, retry semantics and reconciliation.
- `F245-INT-002` — Straight-line depreciation MUST publish downstream intent/event with stable source identity and idempotency key; Procurement, Stock, Manufacturing, Projects, HR or Accounting retain ownership of their private state and may reject/reconcile independently.

## [SPEC-API] Commands, queries and API contracts
- `F245-API-001` — Straight-line depreciation commands MUST define request schema, permission/scope, expected state/version, idempotency where externally visible, domain errors, transaction boundary, audit and downstream effects.
- `F245-API-002` — Straight-line depreciation queries MUST define pagination/filter/sort, as-of/effective-date semantics where relevant, permission-safe aggregates, stable identifiers and compatibility/versioning behavior.
...
- `F245-PERF-001` — Straight-line depreciation MUST define behavior for 0/1/100/10k/1m relevant rows, server pagination/virtualization, batch/background thresholds and latency budgets so enterprise asset registers/history do not degrade into unbounded scans.

## [SPEC-OBSERVABILITY] Logs, metrics, traces, jobs and support diagnostics
- `F245-OBS-001` — Straight-line depreciation MUST emit structured logs/metrics/traces with correlation IDs, job/retry/dead-letter state, business exception counters and reconciliation diagnostics without exposing sensitive financial/custodian data.
...
- `AST-P7-CODE-015` — verified current-code evidence: `services/api/src/modules/accounting/assets.js`.
...
- `F245-E2E-001` — Browser/device E2E MUST prove the primary authorized Straight-line depreciation journey including visible state, persisted data, audit and downstream contract outcome.
- `F245-E2E-002` — E2E MUST prove Straight-line depreciation permission denial, stale/conflict handling, duplicate/retry behavior and reversal/reconciliation or exception recovery without duplicated effect.

## [SPEC-UAT] Human UAT plan
- `F245-UAT-001` — A realistic asset operator MUST execute Straight-line depreciation using documented prerequisites/steps and verify visible, data, audit and downstream outcomes with evidence capture.
- `F245-UAT-002` — A controller/auditor MUST independently verify Straight-line depreciation authorization/SoD, history, calculation/reconciliation and exception/reversal behavior before sign-off.
...
- `F245-SEM-01` — **Authoritative calculation scope and triggering lifecycle**: Define when calculation runs, authoritative owner, provisional/final states and whether recalculation is permitted.
- `F245-SEM-02` — **Inputs, bases, dimensions and effective-dated parameters**: Freeze source inputs, rates/bases, currencies/UOMs, effective dates, source snapshots and lineage.
- `F245-SEM-03` — **Formula, precision, rounding and precedence**: Specify deterministic formulas, precision/scale, rounding order, thresholds, caps/floors and configuration precedence.
- `F245-SEM-04` — **Rate/base visibility and calculation authority**: Define who may configure inputs, run/recalculate, approve/finalize and view sensitive results.
- `F245-SEM-05` — **Explanation, preview, variance and correction experience**: Show inputs, derivation, preview, warnings, variance/explanation, recalculation/correction and accessible output.
- `F245-SEM-06` — **Ledger/stock/payroll/project downstream consequence**: Define authoritative handoff, posting date, idempotency, source reference and reconciliation to downstream truth.
- `F245-SEM-07` — **Backdating, stale inputs, retroactivity and reversal**: Handle changed rates, locked periods, retroactive changes, duplicate runs, partial failure and controlled reversal.
- `F245-SEM-08` — **Golden, property, reconciliation and regression tests**: Require formula goldens, boundary/property tests, precision/serialization tests, reconciliation, E2E and UAT.
...
- Required flow IDs: `F245-PFC-01`, `F245-PFC-02`, `F245-PFC-03`, `F245-PFC-04`, `F245-PFC-05`, `F245-PFC-06`, `F245-PFC-07`, `F245-PFC-08`, `F245-PFC-09`, `F245-PFC-10`
- State transition IDs: `F245-STM-01`, `F245-STM-02`, `F245-STM-03`, `F245-STM-04`, `F245-STM-05`
...
- Curated authoritative benchmark IDs: `PFD-BM-F245-1`; `PFD-BM-F245-2`
- The Pass D mappings are the implementation-planning benchmark authority for **Straight-line depreciation**.
