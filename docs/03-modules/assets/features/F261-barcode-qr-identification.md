# F261 — Barcode / QR identification

## [SPEC-IDENTITY] Identity and traceability
- Canonical ID: `F261`
- Canonical name: **Barcode / QR identification**
- Module: **Assets**
...
Provide unique barcode/QR identity, label lifecycle, scan-to-asset/verify/transfer/maintain field workflows and duplicate prevention.
...
- `AST-P7-BE-061` — official benchmark evidence in `BENCHMARK_REGISTER.csv`.
- `AST-P7-BE-062` — official benchmark evidence in `BENCHMARK_REGISTER.csv`.
...
Independent omission review for **Barcode / QR identification** covered identity uniqueness, hierarchy/component/multi-book implications where relevant, effective dating, thresholds/CIP/partial-transfer/disposal expectations, depreciation convention/rounding, period close, warranty/maintenance/calibration, field/offline evidence, SoD, concurrency, idempotency, reversal/reconciliation, accessibility and scale. No material enterprise expectation is silently omitted because the canonical title is short.

## [SPEC-SUBCAPABILITIES] Sub-capabilities and capability mapping
- `F261-CAP-001` — Barcode / QR identification MUST provide unique barcode/QR identity, label lifecycle, scan-to-asset/verify/transfer/maintain field workflows and duplicate prevention.
- `F261-CAP-002` — Barcode / QR identification MUST define lifecycle/state, effective dating, authorization, approvals, corrections/reversal and immutable history where facts are financial or compliance relevant.
- `F261-CAP-003` — Barcode / QR identification MUST expose auditable public contracts/events and evidence needed for Procurement, Stock, Projects, Manufacturing, HR and Accounting interactions without direct private-table mutation.

## [SPEC-FUNCTIONAL] Functional requirements and user stories
- `F261-FR-001` — Authorized users MUST be able to execute the primary Barcode / QR identification lifecycle with actionable validation, explicit state and durable audit evidence.
- `F261-FR-002` — Users MUST be able to search, filter, inspect exceptions/history/related records for Barcode / QR identification and resolve eligible exceptions without bypassing controls.
- `F261-FR-003` — Approved or posted Barcode / QR identification facts MUST be corrected through controlled amendment/reversal/reopen mechanisms rather than silent destructive edits.
- `F261-US-001` — As an asset operator, I can perform Barcode / QR identification from a focused workspace and understand status, ownership, next action and downstream effect.
- `F261-US-002` — As a controller/auditor, I can prove who changed or approved Barcode / QR identification, why, under which effective date/book/company, and how it reconciles downstream.

## [SPEC-FLOWS] Primary, alternate, exception, retry and reversal flows
- `F261-FLOW-001` — The primary Barcode / QR identification flow MUST validate prerequisites -> authorize -> lock/version relevant records -> apply one atomic domain transition -> audit -> emit idempotent downstream intent -> show outcome.
- `F261-FLOW-002` — Barcode / QR identification MUST cover invalid source, stale version, duplicate request, permission denial, closed period, partial downstream failure, retry, cancellation/reversal and reconciliation without duplicated business effect.
...
- Aggregate owner: Assets domain service for **Barcode / QR identification**.
...
- `F261-DATA-001` — Barcode / QR identification MUST persist stable identifiers, organization/company scope, asset/category/source references, status, effective dates, actor/version and feature-specific values required for unique barcode/QR identity, label lifecycle, scan-to-asset/verify/transfer/maintain field workflows and duplicate prevention.
- `F261-DATA-002` — Barcode / QR identification MUST retain lineage to source procurement/accounting/maintenance/inspection/disposal records, attachments/evidence, approvals, reversals and correlation/idempotency identifiers where applicable.

## [SPEC-VALIDATION] Validation rules
- `F261-VAL-001` — Barcode / QR identification MUST reject malformed identifiers, invalid dates/amounts/units, inactive references, cross-company references, duplicate identities and impossible lifecycle combinations with actionable errors.
- `F261-VAL-002` — Barcode / QR identification MUST validate current asset state, open assignments/maintenance/verification obligations, book/period status and downstream eligibility before committing a controlled transition.

## [SPEC-BUSINESS-RULES] Business rules and invariants
- `F261-BR-001` — Barcode / QR identification MUST obey authoritative asset lifecycle, company/book/effective-date, valuation, custody/maintenance and period rules; AI or UI state cannot override these invariants.
- `F261-BR-002` — Once Barcode / QR identification creates an approved, posted, completed or compliance-significant fact, history MUST remain immutable and corrections MUST link to the original fact.

## [SPEC-CALCULATIONS] Calculations, precision and rounding
- `F261-CALC-001` — Any numeric/date result for Barcode / QR identification MUST define units, currency/precision where relevant, timezone/effective-date boundaries, rounding and reproducibility; no AI-generated value may become authoritative.

## [SPEC-VIEWS] Required view archetypes
- `F261-UX-001` — Desktop UX for Barcode / QR identification MUST expose identity/status, key facts, exception banners, related records, audit and eligible actions without forcing operators through generic CRUD screens.
- `F261-UX-002` — Mobile/tablet UX for Barcode / QR identification MUST support field-relevant scan/search, large touch targets, evidence/photo/document capture where applicable, explicit offline boundary and conflict-safe sync.
- `F261-UX-003` — Barcode / QR identification MUST meet WCAG 2.2 AA intent with semantic labels, keyboard/focus order, announced errors/status, non-color-only state, target sizing and non-drag alternatives.
...
- `F261-AUTO-001` — Background automation for Barcode / QR identification MAY generate due work, schedules, alerts, draft calculations or integration jobs, but MUST use normal domain authorization/system policy, idempotency, audit and reconciliation controls.

## [SPEC-APPROVALS] Approvals, maker-checker and segregation of duties
- `F261-APP-001` — Approval-required Barcode / QR identification transitions MUST snapshot material evidence/values, record approver/reason/time, block prohibited self-approval and invalidate/re-review when material inputs change.

## [SPEC-NOTIFICATIONS] Notifications and communication behavior
- `F261-NOTIF-001` — Barcode / QR identification notifications MUST be event-driven, deduplicated, permission-safe and actionable for due/overdue, exception, approval, expiry/warranty, maintenance/calibration or reconciliation conditions as applicable.
...
- `F261-REP-001` — Barcode / QR identification reporting MUST define metric formulas, as-of/effective-date/book/company scope, permission-safe drilldown, export and reconciliation to authoritative source facts.

## [SPEC-AI] AI opportunities, authority boundary and safeguards
- `F261-AI-001` — AI for Barcode / QR identification MAY extract documents, classify/analyze anomalies, explain schedules or recommend maintenance/verification priorities, but MUST show provenance/uncertainty and MUST NOT authoritatively determine ledger, depreciation, tax, authorization or legal state transitions.

## [SPEC-SECURITY] Security, permissions and field controls
- `F261-SEC-001` — Every Barcode / QR identification query/command MUST enforce server-side organization, company/branch/location/record scope plus action permission; unauthorized totals, finance fields, custodian data and documents MUST not leak.
- `F261-SEC-002` — Sensitive Barcode / QR identification actions MUST enforce maker-checker/SoD policy where configured; creators/requesters/technicians cannot self-approve capitalization, financial posting, impairment/revaluation, disposal or compliance closure when prohibited.
...
- `F261-INT-001` — Barcode / QR identification MUST consume external-module facts only through versioned public contracts with source ID, company, effective date, payload validation, authorization/system trust, retry semantics and reconciliation.
- `F261-INT-002` — Barcode / QR identification MUST publish downstream intent/event with stable source identity and idempotency key; Procurement, Stock, Manufacturing, Projects, HR or Accounting retain ownership of their private state and may reject/reconcile independently.

## [SPEC-API] Commands, queries and API contracts
- `F261-API-001` — Barcode / QR identification commands MUST define request schema, permission/scope, expected state/version, idempotency where externally visible, domain errors, transaction boundary, audit and downstream effects.
- `F261-API-002` — Barcode / QR identification queries MUST define pagination/filter/sort, as-of/effective-date semantics where relevant, permission-safe aggregates, stable identifiers and compatibility/versioning behavior.
...
- `F261-PERF-001` — Barcode / QR identification MUST define behavior for 0/1/100/10k/1m relevant rows, server pagination/virtualization, batch/background thresholds and latency budgets so enterprise asset registers/history do not degrade into unbounded scans.

## [SPEC-OBSERVABILITY] Logs, metrics, traces, jobs and support diagnostics
- `F261-OBS-001` — Barcode / QR identification MUST emit structured logs/metrics/traces with correlation IDs, job/retry/dead-letter state, business exception counters and reconciliation diagnostics without exposing sensitive financial/custodian data.
...
- `AST-P7-CODE-031` — verified current-code evidence: `apps/web/src/modules/assets/components/assets-dashboard.tsx`.
...
- `F261-E2E-001` — Browser/device E2E MUST prove the primary authorized Barcode / QR identification journey including visible state, persisted data, audit and downstream contract outcome.
- `F261-E2E-002` — E2E MUST prove Barcode / QR identification permission denial, stale/conflict handling, duplicate/retry behavior and reversal/reconciliation or exception recovery without duplicated effect.

## [SPEC-UAT] Human UAT plan
- `F261-UAT-001` — A realistic asset operator MUST execute Barcode / QR identification using documented prerequisites/steps and verify visible, data, audit and downstream outcomes with evidence capture.
- `F261-UAT-002` — A controller/auditor MUST independently verify Barcode / QR identification authorization/SoD, history, calculation/reconciliation and exception/reversal behavior before sign-off.
...
- `F261-SEM-01` — **Asset lifecycle from acquisition through disposal**: Define acquired/CIP/capitalized/in-service/held/disposed states, transfers/assignments and effective-dated ownership/custody.
- `F261-SEM-02` — **Asset identity, book/value/location/custody/maintenance history**: Freeze identifiers, category, cost/value, useful life, salvage, books, location/custodian, maintenance and source lineage.
- `F261-SEM-03` — **Capitalization/depreciation/revaluation/maintenance/disposal rules**: Define thresholds/effectivity, depreciation methods/conventions, impairment/revaluation, maintenance/calibration and gain/loss behavior.
- `F261-SEM-04` — **Custody/finance/maintenance authority and SoD**: Separate create/capitalize/transfer/revalue/dispose/post authority and protect financial/sensitive asset data.
- `F261-SEM-05` — **Register, custody, maintenance, verification and disposal workspace**: Cover barcode/QR, field verification, transfer/assignment, maintenance/inspection, financial book view, mobile and accessibility.
- `F261-SEM-06` — **Procurement/hr/projects/accounting contracts**: Create from procurement, link employee/project custody, post depreciation/disposal via public finance contracts and reconcile.
- `F261-SEM-07` — **Partial transfer/disposal, backdating, book lock and missing asset recovery**: Handle concurrent custody changes, retro changes, locked periods, duplicate posting, lost/damaged assets and reversals.
- `F261-SEM-08` — **Depreciation/book/custody/reconciliation verification**: Require calculation goldens, effective-date tests, barcode verification, accounting reconciliation, E2E and UAT.
...
- Required flow IDs: `F261-PFC-01`, `F261-PFC-02`, `F261-PFC-03`, `F261-PFC-04`, `F261-PFC-05`, `F261-PFC-06`, `F261-PFC-07`, `F261-PFC-08`, `F261-PFC-09`, `F261-PFC-10`
- State transition IDs: `F261-STM-01`, `F261-STM-02`, `F261-STM-03`, `F261-STM-04`, `F261-STM-05`
...
- Curated authoritative benchmark IDs: `PFD-BM-F261-1`; `PFD-BM-F261-2`
- The Pass D mappings are the implementation-planning benchmark authority for **Barcode / QR identification**.
