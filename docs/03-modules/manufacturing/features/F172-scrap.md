...
Provide scrap quantity/reason/disposition with material/output/cost impact, approval thresholds and traceability.
...
- `MFG-P5-BE-055` — official benchmark evidence in `BENCHMARK_REGISTER.csv`.
- `MFG-P5-BE-056` — official benchmark evidence in `BENCHMARK_REGISTER.csv`.
...
Independent omission review covered **effectivity/snapshots, multi-level explosion, states, permissions, material/capacity concurrency, shortage/hold, retry/reversal/reconciliation, traceability, costing, responsive shop-floor UX, audit, reporting and scale behavior expected for Scrap**. These expectations are requirements or explicit boundaries; none are silently omitted because the canonical title is short.

## [SPEC-SUBCAPABILITIES] Sub-capabilities and capability mapping
- `F172-CAP-001` — Scrap MUST provide scrap quantity/reason/disposition with material/output/cost impact, approval thresholds and traceability.
- `F172-CAP-002` — Scrap MUST cover lifecycle, permissions, effectivity, concurrency, retry/reversal, audit, reporting, responsive UX and scale; the canonical title is a traceability anchor, not the whole capability.
- `F172-CAP-003` — Scrap MUST preserve Manufacturing ownership of production intent/execution while Stock, Quality, Procurement, Sales, Assets and Accounting effects use versioned public contracts rather than private-table mutation.

## [SPEC-FUNCTIONAL] Functional requirements and user stories
- `F172-FR-001` — The system MUST provide a server-backed Scrap workflow covering success, validation, permission, conflict, shortage/hold/downstream failure, retry/reversal and audit states.
- `F172-FR-002` — Every material Scrap action MUST preserve actor, effective/posting time, reason, source demand/order, structure/routing version, prior/new state or immutable event, and downstream lineage.
- `F172-FR-003` — Scrap MUST remain operational at enterprise BOM/order/transaction volumes using bounded queries, batching/background jobs and stable state transitions as appropriate.
- `F172-US-001` — As an authorized manufacturing engineer/planner/operator, I can perform Scrap with the exact structure, material, resource, time, quality and traceability context needed for the job.
- `F172-US-002` — As a production manager/controller, I can review exceptions, provenance, approvals, variances, history and reconciliation for Scrap within my permitted company/plant/work-centre scope.

## [SPEC-FLOWS] Primary, alternate, exception, retry and reversal flows
- `F172-FLOW-001` — The governed lifecycle for Scrap MUST follow PROPOSED/READY -> POSTED -> RECONCILED, with immutable posting and linked reversal/compensation rather than silent rewrite with explicit guards and no silent historical rewrite.
- `F172-FLOW-002` — Validation, permission, concurrent update, material shortage, quality hold and downstream failures for Scrap MUST be actionable and safely retryable without duplicate stock, WIP, cost, reservation or accounting effects.
...
- Aggregate owner: Manufacturing domain service for **Scrap**.
...
- `F172-DATA-001` — The Scrap model MUST define organization/company/branch/plant, item, BOM/routing/order/operation/resource, warehouse/location, lot/batch/serial and source dimensions as applicable, with stable keys, constraints, indexes, retention and lineage.
- `F172-DATA-002` — Quantity, UOM, dates, rates, version/effectivity, resource, cost, quality and source values that determine historical Scrap interpretation MUST remain reproducible after master/configuration changes.

## [SPEC-VALIDATION] Validation rules
- `F172-VAL-001` — All Scrap commands MUST validate tenant/company scope, item/UOM/quantity/date precision, approved effectivity, lifecycle, resource/material eligibility, tracking/quality rules and cross-reference integrity server-side.
- `F172-VAL-002` — Scrap failures MUST expose stable domain error codes and corrective guidance without leaking unauthorized cost, supplier, customer, employee, cross-company or restricted engineering data.

## [SPEC-BUSINESS-RULES] Business rules and invariants
- `F172-BR-001` — The Manufacturing domain command responsible for Scrap is authoritative; UI, API, import, automation, scanner and AI paths MUST reuse the same deterministic invariant checks.
- `F172-BR-002` — Scrap MUST resolve approved/effective master data at the defined decision point and preserve the resolved version/rates/structure on released or posted production facts so later configuration changes cannot rewrite history.

## [SPEC-CALCULATIONS] Calculations, precision and rounding
- `F172-CALC-001` — All BOM explosion, MRP netting, capacity/time, material scaling, WIP/cost, variance, yield or efficiency calculations applicable to Scrap MUST use deterministic decimal/date rules, explicit units/rates/rounding and reproducible inputs; AI is never authoritative.

## [SPEC-VIEWS] Required view archetypes
- `F172-UX-001` — The Scrap workspace MUST expose identity, lifecycle, source/demand, structure/operation/resource context, material/quality/cost facts as permitted, exceptions, related records and history in one coherent Manufacturing shell.
- `F172-UX-002` — Desktop/tablet/phone views for Scrap MUST define loading, empty, validation, permission, shortage/hold, stale/conflict, offline/retry, destructive confirmation and success states.
- `F172-UX-003` — Shop-floor-heavy Scrap interactions MUST support keyboard and touch operation, scanner/manual alternatives, visible focus, non-color status cues and non-drag alternatives.
...
- `F172-AUTO-001` — Automation MAY create proposals/work for Scrap only through idempotent Manufacturing/public commands; it MUST NOT directly mutate production, stock balance, quality disposition or accounting tables.

## [SPEC-APPROVALS] Approvals, maker-checker and segregation of duties
- `F172-APP-001` — Where policy requires approval for BOM/routing release, engineering change, overproduction, material/cost exception, quality release or high-impact Scrap action, approval MUST bind to the exact version/state/value being approved and prevent self-approval where configured.

## [SPEC-NOTIFICATIONS] Notifications and communication behavior
- `F172-NOTIF-001` — Notifications for Scrap MUST be event-driven, deduplicated, preference/permission aware and deep-link to the exact shortage, hold, delay, variance or work item without exposing restricted data.
...
- `F172-REP-001` — Reports/KPIs for Scrap MUST define grain, filters, freshness/as-of semantics, drilldown and reconciliation to authoritative production, Stock, Quality and Accounting records.

## [SPEC-AI] AI opportunities, authority boundary and safeguards
- `F172-AI-001` — AI for Scrap is limited to assist/recommend/generate workflows with provenance, freshness, uncertainty, explanation and permission filtering; AI MUST NOT be authoritative for BOM effectivity, MRP math, stock/WIP quantity, quality hold/release, costing, posting legality or authorization.

## [SPEC-SECURITY] Security, permissions and field controls
- `F172-SEC-001` — Every Scrap query and mutation MUST enforce authentication, module entitlement, organization/company/branch/plant/work-centre scope, action permission and record visibility server-side.
- `F172-SEC-002` — Engineering structures, costs/rates, employee time, supplier/customer references and approvals in Scrap MUST be field/action restricted and protected against IDOR, aggregate leakage and unauthorized export.
...
- `F172-INT-001` — Inbound cross-module requests affecting Scrap MUST call a versioned Manufacturing public command with authorization, idempotency/source reference, validation and explicit success/failure result.
- `F172-INT-002` — Outbound effects from Scrap MUST use public commands/events or orchestration with retry, reversal/compensation and reconciliation; Manufacturing MUST NOT mutate another module’s private tables.

## [SPEC-API] Commands, queries and API contracts
- `F172-API-001` — Scrap mutation APIs MUST define request schema, authorization, idempotency, expected state/version, stable errors, audit/outbox effects and compatibility semantics.
- `F172-API-002` — Scrap query APIs MUST define dimension grain, pagination/filter/sort, as-of/freshness semantics, authorization-safe aggregates and stable response contracts.
...
- `F172-PERF-001` — Scrap MUST define performance envelopes for 0/1/100/10k/1m BOM lines, requirements, operations or production transactions as applicable, with bounded synchronous work and explicit async thresholds.

## [SPEC-OBSERVABILITY] Logs, metrics, traces, jobs and support diagnostics
- `F172-OBS-001` — Scrap MUST emit structured logs/metrics/traces for failures, retries, lock contention, MRP/job lag, shortages, hold blocks, drift/reconciliation and downstream delivery using correlation IDs.
...
- Verified current-code evidence: `MFG-P5-CODE-028` → `services/api/src/modules/manufacturing/index.js`.
...
- `F172-E2E-001` — E2E MUST prove an authorized persona can complete the primary Scrap journey and observe correct structure/material/resource/state/history/downstream effects.
- `F172-E2E-002` — E2E MUST cover unauthorized access, stale/concurrent or replayed action and at least one relevant shortage, hold, capacity conflict, partial, reversal, rework or reconciliation path for Scrap.

## [SPEC-UAT] Human UAT plan
- `F172-UAT-001` — A real manufacturing engineer/planner/supervisor/operator MUST execute the primary Scrap job with realistic data on the appropriate desktop/tablet/shop-floor form factor, capturing visible and data evidence.
- `F172-UAT-002` — A production manager plus relevant Stock/Quality/Procurement/Sales/Maintenance/Finance persona MUST verify permissions, exceptions, reversal/reconciliation, audit and cross-module outcomes for Scrap.
...
- `F172-SEM-01` — **Production definition/execution lifecycle**: Define version/effectivity or planned/released/in-progress/completed/closed states appropriate to the feature.
- `F172-SEM-02` — **Material, operation, resource, quantity and revision lineage**: Freeze BOM/routing/order/resource links, UOM, revisions, batches/serials, time/cost inputs and genealogy.
- `F172-SEM-03` — **Material/capacity/sequence/yield/cost rules**: Define availability, reservations, sequencing, capacity calendars, consumption, yield/scrap and deterministic costing behavior.
- `F172-SEM-04` — **Planner/operator/supervisor authority and plant scope**: Enforce plant/work-centre/order scope, release/close/rework authority and maker-checker where financially material.
- `F172-SEM-05` — **Planning/shop-floor execution and exception workspace**: Cover planning boards, job/work views, scan/issue/consume/complete actions, downtime/rework, mobile/tablet and accessibility.
- `F172-SEM-06` — **Stock/quality/assets/procurement/accounting contracts**: Define public movements, quality gates, maintenance impacts, subcontracting and costing/accounting handoffs.
- `F172-SEM-07` — **Shortage, capacity conflict, partial completion, rework and reversal**: Handle shortage/race, over/under consumption, failed operations, rework/scrap, cancellation and reconciliation.
- `F172-SEM-08` — **MRP/material/genealogy/cost invariant verification**: Require deterministic planning/cost goldens, concurrency, traceability, integration journeys and UAT.
...
- Required flow IDs: `F172-PFC-01`, `F172-PFC-02`, `F172-PFC-03`, `F172-PFC-04`, `F172-PFC-05`, `F172-PFC-06`, `F172-PFC-07`, `F172-PFC-08`, `F172-PFC-09`, `F172-PFC-10`
- State transition IDs: `F172-STM-01`, `F172-STM-02`, `F172-STM-03`, `F172-STM-04`, `F172-STM-05`
...
- Curated authoritative benchmark IDs: `PFD-BM-F172-1`; `PFD-BM-F172-2`
