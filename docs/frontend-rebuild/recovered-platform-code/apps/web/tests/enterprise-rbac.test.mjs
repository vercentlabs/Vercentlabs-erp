// PARKED — see docs/frontend-rebuild/recovered-platform-code/README.md.
// This test verifies the parked access-control.ts snapshot is internally
// consistent with the live packages/permissions package; it is NOT
// evidence that RBAC enforcement works in production (access-control.ts
// isn't wired into any active build). Path resolution below is adapted
// from the original (which assumed it ran via `pnpm --filter web exec`
// with cwd=apps/web) to work from this parked location instead — the
// original file otherwise unchanged.
...
const webRoot = path.resolve(here, ".."); // .../recovered-platform-code/apps/web
const root = path.resolve(here, "../../../../../.."); // repo root
// Resolve `typescript` through apps/web's real node_modules (it isn't
// reachable by upward-walking from this parked location), not this file's.
...
test("web and mobile access administration use cumulative roles and effective dates", () => {
  const userRoute = read("docs/frontend-rebuild/recovered-platform-code/apps/web/src/app/api/users/[userId]/route.ts");
  const invitationRoute = read("docs/frontend-rebuild/recovered-platform-code/apps/web/src/app/api/invitations/route.ts");
  const acceptanceRoute = read(
    "docs/frontend-rebuild/recovered-platform-code/apps/web/src/app/api/invitations/accept/route.ts",
  );
  const auth = read("docs/frontend-rebuild/recovered-platform-code/apps/web/src/core/auth.ts");
  const webUi = read("docs/frontend-rebuild/recovered-platform-code/apps/web/src/core/components/user-administration.tsx");
...
test("delegated administrators cannot modify users or invitations with wider company scope", () => {
  const administration = read("docs/frontend-rebuild/recovered-platform-code/apps/web/src/core/access-admin.ts");
  assert.match(
    administration,
...
test("role catalogue visibility is separate from role mutation", () => {
  const rolePage = read("docs/frontend-rebuild/recovered-platform-code/apps/web/src/app/(app)/settings/roles/page.tsx");
  ...
  const shell = read("docs/frontend-rebuild/recovered-platform-code/apps/web/src/core/navigation/administration.ts");
  const settings = read("docs/frontend-rebuild/recovered-platform-code/apps/web/src/app/(app)/settings/page.tsx");
