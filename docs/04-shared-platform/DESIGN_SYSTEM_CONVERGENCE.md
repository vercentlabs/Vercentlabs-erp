...
### Go 4 implementation status

`GO4_COMPLETE` — the shared Experience Kernel now owns explicit list/work-queue,
Record 360, transaction-document, board and operations-workspace archetypes.
Representative production screens consume those contracts without reimplementing
their loaders, commands or domain services. The approval queue also moves from a
page-owned raw table to the responsive `EnterpriseDataGrid`.

Authenticated ERP browser gates are defined in `apps/web/playwright.erp.config.ts`
and cover desktop/tablet/mobile/compact overflow, reduced-motion rendering,
WCAG 2.2 AA-oriented axe scans and screenshot regression for representative
authenticated routes. The gate is intentionally fail-closed: it requires a
dedicated non-production E2E account plus stable lead and sales-order fixture
IDs, and visual baselines are never auto-regenerated by the convergence
installer.

No database schema, API contract, permission model or domain state transition is
changed by Go 4.
