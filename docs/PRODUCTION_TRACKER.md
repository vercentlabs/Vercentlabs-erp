...
**Real but scoped gap found, recorded for the Sales gap-closing pass:** `getSalesOptions`'s customer picker query has no `LIMIT` (unlike its own `opportunities` query, which caps at 200) — an org with a large customer base loads its entire active list on every quotation/order create. Volume/performance issue, not a security one.
