...
- **F042 Sales orders (priority item, cross-module boundary violation touching CRM's own audit trail):** `confirmSalesOrder` closes the source CRM opportunity with a raw `UPDATE tenant.crm_opportunities SET status='won'...` instead of calling CRM's own `moveOpportunityStage` — bypasses the stage-history snapshotting (`status`/`outcome_reason_id`/`outcome_reason_label`) the CRM gap-closing pass just finished making complete for every other opportunity-close path (commit `6c384513`). No reversal either: cancelling the confirming order later never reopens the opportunity, since the win itself never went through the governed path that has a reopen counterpart.
- **F045 Availability check: the entire capability is missing.** Zero references anywhere in Sales to stock balances/availability — a quotation or order can be created and confirmed for an item with zero real stock, with no warning shown. Stock already exposes a ready-made public `getStockAvailability` function; Sales simply never calls it. Pure wiring gap on a capability that doesn't exist yet at all, not a refinement of an existing one.
- **F046 Stock reservation (likely the most serious functional finding in Sales): "reservation" is a local illusion, not real stock reservation.** `reserveSalesOrderLines` only writes to Sales' own private `sales_order_line_progress.reserved_quantity` — it never touches `tenant.stock_balances.reserved_quantity`, the real, shared ledger Stock/Manufacturing/POS/Procurement all actually read and update (confirmed: zero imports from `modules/stock` anywhere in Sales). A confirmed Sales order that believes it has reserved stock has not actually removed that quantity from what any other module sees as available — nothing stops another sales order, a POS sale, or a stock transfer from consuming the same physical inventory. Also no release-on-cancel path. Needs real integration with Stock's canonical reservation path, not a small patch. **This is a documented architecture requirement, not an inferred one** — `docs/04-cross-module/SALES_TO_STOCK_FULFILMENT.md` explicitly states "Sales confirms demand and calls Stock availability/reservation contracts. Stock owns reservation, picking eligibility and physical issue" — that contract was simply never implemented (F045/F046/F047 are all facets of the same missing integration).
- F047 Partial fulfilment: same root cause as F046 — `completeFulfillmentRequest` never calls Stock's canonical `postStockMovement`; fulfilling a Sales order updates only Sales' own bookkeeping with no real inventory effect. Fix these two together, they're one integration gap. Smaller gap also found: no delivery tolerance or remainder-cancellation action.
- F048 Backorders: no entity exists anywhere (zero code matches for "backorder"). Confirms the pattern — F045/F046/F047/F048 are four feature-level symptoms of one unimplemented integration, the contract `docs/04-cross-module/SALES_TO_STOCK_FULFILMENT.md` already describes but was never built. Treat as one gap-closing effort, not four.
