...
## Scope
Modules: CRM; Sales; Stock / Inventory; Accounting / Finance. Outcome: lead/opportunity/order/invoice/receipt chain.

## Canonical sequence
1. **lead capture/qualification** — owned by the module that owns the aggregate; cross-module effects use public commands/events only.
2. **opportunity/quotation** — owned by the module that owns the aggregate; cross-module effects use public commands/events only.
3. **sales order** — owned by the module that owns the aggregate; cross-module effects use public commands/events only.
4. **reservation/fulfilment** — owned by the module that owns the aggregate; cross-module effects use public commands/events only.
5. **invoice** — owned by the module that owns the aggregate; cross-module effects use public commands/events only.
6. **receipt/allocation** — owned by the module that owns the aggregate; cross-module effects use public commands/events only.
...
## Verification IDs
- E2E: `ENT-LEAD_TO_CASH-E2E-001` happy path; `ENT-LEAD_TO_CASH-E2E-002` permission/failure; `ENT-LEAD_TO_CASH-E2E-003` retry/reversal/reconciliation.
- UAT: `ENT-LEAD_TO_CASH-UAT-001` operator journey with visible state and downstream reconciliation.
