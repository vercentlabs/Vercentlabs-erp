...
## Scope
Modules: Assets; Procurement; Accounting / Finance. Outcome: acquisition to disposal and books.

## Canonical sequence
1. **purchase/capitalization** — owned by the module that owns the aggregate; cross-module effects use public commands/events only.
2. **asset recognition** — owned by the module that owns the aggregate; cross-module effects use public commands/events only.
3. **depreciation/revaluation/impairment** — owned by the module that owns the aggregate; cross-module effects use public commands/events only.
4. **transfer/maintenance** — owned by the module that owns the aggregate; cross-module effects use public commands/events only.
5. **sale/scrap/disposal** — owned by the module that owns the aggregate; cross-module effects use public commands/events only.
6. **gain/loss and GL reconciliation** — owned by the module that owns the aggregate; cross-module effects use public commands/events only.
...
## Verification IDs
- E2E: `ENT-ASSET_TO_BOOKS-E2E-001` happy path; `ENT-ASSET_TO_BOOKS-E2E-002` permission/failure; `ENT-ASSET_TO_BOOKS-E2E-003` retry/reversal/reconciliation.
- UAT: `ENT-ASSET_TO_BOOKS-UAT-001` operator journey with visible state and downstream reconciliation.
