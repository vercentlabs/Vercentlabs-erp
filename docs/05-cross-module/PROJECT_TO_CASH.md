...
## Scope
Modules: Projects; Sales; HR & Payroll; Procurement; Accounting / Finance. Outcome: project delivery to billing and margin.

## Canonical sequence
1. **project/contract setup** — owned by the module that owns the aggregate; cross-module effects use public commands/events only.
2. **work/time/expense/procurement actuals** — owned by the module that owns the aggregate; cross-module effects use public commands/events only.
3. **billing eligibility** — owned by the module that owns the aggregate; cross-module effects use public commands/events only.
...
6. **profitability reconciliation** — owned by the module that owns the aggregate; cross-module effects use public commands/events only.
...
## Verification IDs
- E2E: `ENT-PROJECT_TO_CASH-E2E-001` happy path; `ENT-PROJECT_TO_CASH-E2E-002` permission/failure; `ENT-PROJECT_TO_CASH-E2E-003` retry/reversal/reconciliation.
- UAT: `ENT-PROJECT_TO_CASH-UAT-001` operator journey with visible state and downstream reconciliation.
