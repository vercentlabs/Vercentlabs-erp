...
## Scope
Modules: Manufacturing; Quality; Stock / Inventory. Outcome: production result through quality disposition.

## Canonical sequence
1. **inspection trigger** — owned by the module that owns the aggregate; cross-module effects use public commands/events only.
2. **sample/result** — owned by the module that owns the aggregate; cross-module effects use public commands/events only.
3. **hold if nonconforming** — owned by the module that owns the aggregate; cross-module effects use public commands/events only.
4. **disposition/rework/scrap/release** — owned by the module that owns the aggregate; cross-module effects use public commands/events only.
5. **atomic stock availability update** — owned by the module that owns the aggregate; cross-module effects use public commands/events only.
...
## Verification IDs
- E2E: `ENT-MANUFACTURING_QUALITY_STOCK-E2E-001` happy path; `ENT-MANUFACTURING_QUALITY_STOCK-E2E-002` permission/failure; `ENT-MANUFACTURING_QUALITY_STOCK-E2E-003` retry/reversal/reconciliation.
- UAT: `ENT-MANUFACTURING_QUALITY_STOCK-UAT-001` operator journey with visible state and downstream reconciliation.
