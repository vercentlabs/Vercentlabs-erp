...
## Scope
Modules: Procurement; Stock / Inventory; Quality; Accounting / Finance. Outcome: purchase commitment to supplier settlement.

## Canonical sequence
1. **requisition/approval** — owned by the module that owns the aggregate; cross-module effects use public commands/events only.
2. **sourcing/PO** — owned by the module that owns the aggregate; cross-module effects use public commands/events only.
3. **receipt/quality** — owned by the module that owns the aggregate; cross-module effects use public commands/events only.
4. **supplier invoice/match** — owned by the module that owns the aggregate; cross-module effects use public commands/events only.
5. **payment** — owned by the module that owns the aggregate; cross-module effects use public commands/events only.
6. **bank/GL reconciliation** — owned by the module that owns the aggregate; cross-module effects use public commands/events only.
...
## Verification IDs
- E2E: `ENT-PROCURE_TO_PAY-E2E-001` happy path; `ENT-PROCURE_TO_PAY-E2E-002` permission/failure; `ENT-PROCURE_TO_PAY-E2E-003` retry/reversal/reconciliation.
- UAT: `ENT-PROCURE_TO_PAY-UAT-001` operator journey with visible state and downstream reconciliation.
