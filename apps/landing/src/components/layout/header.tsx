...
const navigationLinks = [
  { label: "Product", href: "/#platform-preview" },
      ...
      {menuOpen ? (
        <>
          <button
            ...
            onClick={closeMenu}
            className="fixed inset-0 top-[72px] z-[-1] bg-slate-950/30 backdrop-blur-[2px] xl:hidden"
          />

          <div
            id="mobile-navigation"
            className="absolute inset-x-0 top-full max-h-[calc(100vh-72px)] overflow-y-auto border-b border-slate-200 bg-white shadow-2xl xl:hidden"
          >
            <PageContainer className="py-4">
                ...
                {navigationLinks.map((item) => {
                  ...
                  return (
                    <Link
                      ...
                      aria-current={active ? "page" : undefined}
                      className={
                        "flex min-h-[52px] items-center justify-between rounded-xl px-4 text-sm font-bold transition " +
                        (active
                          ? "bg-indigo-50 text-indigo-700"
              ...
              <div className="mt-4 grid gap-2 border-t border-slate-200 pt-4 sm:grid-cols-2">
                <a
                  href={siteConfig.appUrl}
                  onClick={closeMenu}
                  className="button-secondary min-h-[52px] w-full"
                >
                  Sign in
                </a>
                <Link
                  href="/contact"
                  onClick={closeMenu}
                  className="button-primary min-h-[52px] w-full"
                >
                  Book a demo
                </Link>
              </div>
