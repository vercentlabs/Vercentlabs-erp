...
test("BOM definition, second-person approval, multi-level explosion, where-used, engineering change", async ({ browser }) => {
  ...
  try {
    ...
    const change = m.getByRole("row", { name: new RegExp(`ECN-.*Add a third component ${items.compC.code}.*Draft`) });
    ...
    await m.getByRole("row", { name: new RegExp(`ECN-.*Add a third component ${items.compC.code}.*Submitted`) }).getByRole("button", { name: "Approve" }).click();
    ...
    await a.getByRole("row", { name: new RegExp(`ECN-.*Add a third component ${items.compC.code}.*Submitted`) }).getByRole("button", { name: "Approve" }).click();
    await expect(a.getByText("Change approved.")).toBeVisible({ timeout: 30_000 });
    await m.reload({ waitUntil: "domcontentloaded" });
    await m.getByRole("row", { name: new RegExp(`ECN-.*Add a third component ${items.compC.code}.*Approved`) }).getByRole("button", { name: "Implement" }).click();
    await expect(m.getByText(/a new BOM version is active/)).toBeVisible({ timeout: 30_000 });
    await m.getByRole("row", { name: new RegExp(`ECN-.*Add a third component ${items.compC.code}.*Implemented`) }).getByRole("link", { name: "New version" }).click();
