...
import { ingressTransaction } from "@/core/db";
...
export async function POST(request: Request) {
  try {
    ...
    // ingressTransaction(), not withIngressClient() — the token claim and the user's
    // email_verified_at update must commit or roll back together.
    const result = await ingressTransaction((client) => consumeEmailVerificationToken(client, body.token));
