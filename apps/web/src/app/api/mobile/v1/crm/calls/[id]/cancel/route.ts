...
import { callLifecycleSchema } from "@/modules/crm/crm-data-operations-and-customization/input-validation";
...
export async function POST(request: Request, route: { params: Promise<{ id: string }> }) {
  try {
    ...
    const response = await tenantTransaction(context.organizationId, (client) => withMobileIdempotency(client, session, request, input, async () => {
      const before = await getCrmCall(client, context, id); const record = await cancelCrmCall(client, context, id, input);
      if (!record.replayed) await audit({ organizationId: context.organizationId, actorUserId: session.userId, eventType: "crm.call.cancelled", entityType: "call", entityId: id, beforeData: crmCallAuditSnapshot(before), afterData: crmCallAuditSnapshot(record), request, client });
      return { message: record.replayed ? "Call is already cancelled." : "Call cancelled.", record };
    }));
