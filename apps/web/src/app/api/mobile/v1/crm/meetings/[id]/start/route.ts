...
export async function POST(request: Request, route: { params: Promise<{ id: string }> }) {
  try {
    ...
    const response = await tenantTransaction(context.organizationId, (client) => withMobileIdempotency(client, session, request, input, async () => {
      const before = await getCrmMeeting(client, context, id); const record = await startCrmMeeting(client, context, id, input);
      if (!record.replayed) await audit({ organizationId: context.organizationId, actorUserId: session.userId, eventType: "crm.meeting.started", entityType: "meeting", entityId: id, beforeData: crmMeetingAuditSnapshot(before), afterData: crmMeetingAuditSnapshot(record), request, client });
      return { message: record.replayed ? "Meeting is already in progress." : "Meeting started.", record };
    }));
