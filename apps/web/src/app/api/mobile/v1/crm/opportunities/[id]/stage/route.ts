...
export async function POST(request: Request, route: { params: Promise<{ id: string }> }) {
  try {
    ...
    const response = await tenantTransaction(context.organizationId, async (client) => withMobileIdempotency(client, session, request, input, async () => {
      const record = await moveOpportunityStage(client, context, id, input.stageId, input.note, { expectedUpdatedAt: input.expectedUpdatedAt, expectedStageId: input.expectedStageId, outcomeReasonId: input.outcomeReasonId, outcomeNotes: input.outcomeNotes });
