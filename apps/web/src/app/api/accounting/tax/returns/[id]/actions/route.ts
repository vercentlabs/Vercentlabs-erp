import { updateTaxReturnStatus } from "@vercentlabs/api";
...
export async function POST(
  ...
  try {
    ...
    const { session, context: accountingContext } = await accountingSession(true);
    ...
    const taxReturn = await tenantTransaction(
      ...
      async (client) => {
        const updated = await updateTaxReturnStatus(client, accountingContext, id, input);
        await audit({
