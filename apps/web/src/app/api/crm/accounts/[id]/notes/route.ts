...
// F017 closeout — Account previously had no Notes support at all. Same
// canonical Notes domain module Lead already uses (notes-operations.js),
// entityType 'party' (the internal name for Account).
export async function GET(request: Request, { params }: Params) {
  try {
    ...
    const notes = await tenantTransaction(context.organizationId, (client) =>
      listCrmNotes(client, context, "party", id, { includeArchived }),
    );
    return ok({ notes });
  } catch (error) {
...
export async function POST(request: Request, { params }: Params) {
  try {
    ...
    const note = await tenantTransaction(context.organizationId, async (client) => {
      const created = await createCrmNote(client, context, "party", id, input);
      await audit({
        organizationId: context.organizationId,
        actorUserId: session.userId,
        eventType: "crm.account.note_created",
        entityType: "party",
        entityId: id,
        afterData: created,
        request,
