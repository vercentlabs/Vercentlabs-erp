import { cancelCrmMeeting, getCrmMeeting } from "@vercentlabs/api";
...
export async function POST(request: Request, route: { params: Promise<{ id: string }> }) {
  try {
    ...
    const record = await tenantTransaction(context.organizationId, async (client) => {
      ...
      const cancelled = await cancelCrmMeeting(client, context, id, input);
      if (!cancelled.replayed) await audit({ organizationId: context.organizationId, actorUserId: context.userId, eventType: "crm.meeting.cancelled", entityType: "meeting", entityId: id, beforeData: crmMeetingAuditSnapshot(before), afterData: crmMeetingAuditSnapshot(cancelled), request, client });
      return cancelled;
    });
    return ok({ message: record.replayed ? "Meeting is already cancelled." : "Meeting cancelled.", record });
