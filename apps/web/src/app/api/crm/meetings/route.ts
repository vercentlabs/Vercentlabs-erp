...
import { createMeetingSchema } from "@/modules/crm/crm-data-operations-and-customization/input-validation";
...
export async function GET(request: Request) {
  try {
    ...
    const result = await tenantTransaction(context.organizationId, (client) =>
      listCrmMeetings(client, context, {
...
export async function POST(request: Request) {
  try {
    ...
    const record = await tenantTransaction(context.organizationId, async (client) => {
      const created = await createCrmMeeting(client, context, input);
      await audit({
        organizationId: context.organizationId,
        actorUserId: context.userId,
        eventType: input.mode === "log" ? "crm.meeting.logged" : "crm.meeting.scheduled",
        entityType: "meeting",
        entityId: String(created.id),
        afterData: crmMeetingAuditSnapshot(created),
        request,
        client,
      });
      return created;
    });
    return ok({ message: input.mode === "log" ? "Meeting logged." : "Meeting scheduled.", record }, 201);
  } catch (error) {
    return crmErrorResponse(error);
  }
}
