...
// F017 closeout — Contact previously had no governed attachment support at
// all. Same canonical attachment domain module Lead/Account already use
// (attachments-operations.js), entityType 'contact'.
export async function GET(_request: Request, { params }: Params) {
  try {
    ...
    const attachments = await tenantTransaction(context.organizationId, (client) => listCrmAttachments(client, context, "contact", id));
...
export async function POST(request: Request, { params }: Params) {
  try {
    ...
    const attachment = await tenantTransaction(context.organizationId, async (client) => {
      const created = await createCrmAttachment(client, context, "contact", id, {
        id: attachmentId,
      ...
      await audit({
        ...
        eventType: "crm.contact.attachment_uploaded",
        entityType: "contact",
        entityId: id,
        afterData: created,
        request,
