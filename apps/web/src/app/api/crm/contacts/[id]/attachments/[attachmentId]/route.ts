...
export async function GET(_request: Request, { params }: Params) {
  try {
    ...
    const row = await tenantTransaction(context.organizationId, (client) => getCrmAttachmentContent(client, context, "contact", id, attachmentId));
...
export async function DELETE(request: Request, { params }: Params) {
  try {
    ...
    await tenantTransaction(context.organizationId, async (client) => {
      const deleted = await deleteCrmAttachment(client, context, "contact", id, attachmentId);
      await audit({
        organizationId: context.organizationId,
        actorUserId: session.userId,
        eventType: "crm.contact.attachment_deleted",
        entityType: "contact",
        entityId: id,
        beforeData: deleted,
        request,
