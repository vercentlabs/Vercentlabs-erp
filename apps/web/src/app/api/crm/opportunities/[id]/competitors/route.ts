import { listOpportunityCompetitors, addOpportunityCompetitor } from "@vercentlabs/api";
...
export async function GET(_request: Request, route: Route) {
  try {
    ...
    return ok({
      rows: await tenantTransaction(context.organizationId, (client) => listOpportunityCompetitors(client, context, id)),
    });
  } catch (error) {
...
export async function POST(request: Request, route: Route) {
  try {
    ...
    const record = await tenantTransaction(context.organizationId, async (client) => {
      const created = await addOpportunityCompetitor(client, context, id, input);
      await audit({
        organizationId: context.organizationId,
        actorUserId: context.userId,
        eventType: "crm.opportunity_competitor.added",
        entityType: "opportunity",
        entityId: id,
        afterData: { competitorId: created.competitorId, isPrimary: created.isPrimary },
        request,
        client,
      });
      return created;
    });
    return ok({ message: "Competitor added.", record }, 201);
  } catch (error) {
    return crmErrorResponse(error);
  }
}
