...
// F017 closeout — Opportunity previously had no governed attachment
// support at all. Same canonical attachment domain module Lead/Account/
// Contact already use (attachments-operations.js). Sensitive-content gate
// reuses crmLeadsViewSensitive — the same established precedent
// opportunity-detail-data.ts's own canSeeOpportunitySensitiveContent
// already uses for this entity type.
export async function GET(_request: Request, { params }: Params) {
  try {
    ...
    const attachments = await tenantTransaction(context.organizationId, (client) => listCrmAttachments(client, context, "opportunity", id));
...
export async function POST(request: Request, { params }: Params) {
  try {
    ...
    const attachment = await tenantTransaction(context.organizationId, async (client) => {
      const created = await createCrmAttachment(client, context, "opportunity", id, {
        id: attachmentId,
      ...
      await audit({
        ...
        eventType: "crm.opportunity.attachment_uploaded",
