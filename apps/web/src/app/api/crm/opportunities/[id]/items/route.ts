import { listOpportunityItems, addOpportunityItem } from "@vercentlabs/api";
...
export async function GET(_request: Request, route: Route) {
  try {
    ...
    return ok({
      rows: await tenantTransaction(context.organizationId, (client) => listOpportunityItems(client, context, id)),
    });
  } catch (error) {
...
export async function POST(request: Request, route: Route) {
  try {
    ...
    const record = await tenantTransaction(context.organizationId, async (client) => {
      const created = await addOpportunityItem(client, context, id, input);
      await audit({
        organizationId: context.organizationId,
        actorUserId: context.userId,
        eventType: "crm.opportunity_item.added",
        entityType: "opportunity",
        entityId: id,
        afterData: { opportunityItemId: created.id, itemId: created.itemId, quantity: created.quantity, unitPrice: created.unitPrice },
        request,
        client,
      });
      return created;
    });
    return ok({ message: "Product added.", record }, 201);
  } catch (error) {
    return crmErrorResponse(error);
  }
}
