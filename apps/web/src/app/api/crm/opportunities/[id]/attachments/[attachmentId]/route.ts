...
export async function GET(_request: Request, { params }: Params) {
  try {
    ...
    const row = await tenantTransaction(context.organizationId, (client) => getCrmAttachmentContent(client, context, "opportunity", id, attachmentId));
...
export async function DELETE(request: Request, { params }: Params) {
  try {
    ...
    await tenantTransaction(context.organizationId, async (client) => {
      const deleted = await deleteCrmAttachment(client, context, "opportunity", id, attachmentId);
      await audit({
        organizationId: context.organizationId,
        actorUserId: session.userId,
        eventType: "crm.opportunity.attachment_deleted",
