...
// recurrence), never duplicated here.
export async function GET(request: Request) {
  try {
    ...
    const result = await withClient((client) => listCrmTasks(client, crmContext(session), filters));
    return ok(result);
  } catch (error) {
...
export async function POST(request: Request) {
  try {
    ...
    const record = await tenantTransaction(session.organizationId, (client) => createCrmTask(client, crmContext(session), input));
