...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const record = await tenantTransaction(context.organizationId, async (client) => {
      const result = await startCrmTask(client, context, id, input);
      await audit({ organizationId: context.organizationId, actorUserId: session.userId, eventType: "crm.task.started", entityType: "task", entityId: id, metadata: { status: result.status, assignedTo: result.assignedTo, dueAt: result.dueAt }, request, client });
      return result;
    });
    return ok({ message: "Task started.", record });
