...
import VendorMatchForm from "@/modules/accounting/components/vendor-match-form";
...
export default async function PayableDetailPage({
  ...
  try {
    ({ data, governance, timeline } = await tenantTransaction(
      ...
      async (client) => {
        const detail = (await getVendorBill(
          client,
          context,
          id,
        )) as unknown as Omit<Detail, "match">;
        const match = (await getVendorBillMatch(
          client,
          context,
          id,
        )) as Row | null;
        return {
          data: { ...detail, match },
          governance: (await assessVendorBillReadiness(
            client,
            context,
            id,
          )) as unknown as Governance,
          timeline: (await getVendorBillGovernanceTimeline(
            client,
            context,
            id,
          )) as unknown as Row[],
  ...
  const governedBill = governance.bill;
  ...
  return (
    <>
      <section className="page-heading">
        <div>
          <p className="eyebrow">
            {isCreditNote ? "Vendor credit note" : "Vendor bill"}
          </p>
          <h1>{String(bill.bill_number)}</h1>
          <p>
            {String(bill.supplier_name)} · Vendor reference{" "}
      ...
      <div className="accounting-action-bar">
        ...
        hasPermission(session, PERMISSIONS.accountingPayablesManage) ? (
          <AccountingActionButton
            endpoint={`/api/accounting/payables/bills/${id}/actions`}
            action="submit"
            label="Submit for approval"
            tone="secondary"
          />
        ) : null}
        {String(bill.status) === "approved" &&
        hasPermission(session, PERMISSIONS.accountingPayablesManage) ? (
          <AccountingActionButton
            endpoint={`/api/accounting/payables/bills/${id}/actions`}
            action="post"
            label={isCreditNote ? "Post vendor credit" : "Post vendor bill"}
            tone="primary"
          />
        ) : null}
      </div>
      ...
      <section className="panel">
        <p className="eyebrow">Payables governance</p>
        <h2>Matching, payment and exception readiness</h2>
        <section className="accounting-capability-grid">
          {[
            ["Readiness", health.readiness || "unknown"],
            ["Risk", health.riskBand || "unknown"],
            ["Payment", health.paymentEligible ? "eligible" : "not eligible"],
            ["Match", governedBill.matching_status || "not required"],
            ["Exception", governedBill.exception_case_status || "not opened"],
          ].map(([label, value]) => (
      ...
      <div className="accounting-two-column">
        <section className="panel">
          ...
          <h2>Expense and input-tax detail</h2>
          <div className="accounting-table accounting-invoice-detail">
            ...
            {data.lines.map((line) => (
              <div className="accounting-table-row" key={String(line.id)}>
                ...
                <span>
                  {String(line.expense_account_code)} ·{" "}
                  {String(line.expense_account_name)}
                </span>
                <span>
                  {String(bill.currency_code)} {String(line.net_amount)}
                </span>
                <span>
                  {String(bill.currency_code)} {String(line.tax_amount)}
                </span>
                <span>
                  {String(bill.currency_code)} {String(line.line_total)}
                </span>
              </div>
            ))}
          </div>
        ...
        <aside className="panel accounting-summary">
          ...
          <dl>
            <div>
              <dt>Subtotal</dt>
              <dd>
                {String(bill.currency_code)} {String(bill.subtotal)}
              </dd>
            </div>
            <div>
              <dt>Discount</dt>
              <dd>
                {String(bill.currency_code)} {String(bill.discount_total)}
              </dd>
            </div>
            <div>
              <dt>Tax</dt>
              <dd>
                {String(bill.currency_code)} {String(bill.tax_total)}
              </dd>
            </div>
            <div>
              <dt>Grand total</dt>
              <dd>
                <strong>
                  {String(bill.currency_code)} {String(bill.grand_total)}
                </strong>
              </dd>
            </div>
            <div>
              ...
              <dd>
                <strong>
                  {String(bill.currency_code)} {String(bill.outstanding_amount)}
                </strong>
              </dd>
            </div>
          </dl>
      ...
      {canAllocate ? (
        <section className="panel">
          ...
          <h2>Apply credit to an open vendor bill</h2>
          <CreditAllocationForm
            endpoint={`/api/accounting/payables/bills/${id}/actions`}
            kind="payable"
            creditBalance={String(bill.outstanding_amount)}
      ...
      {!isCreditNote &&
      ["draft", "approved"].includes(String(bill.status)) &&
      hasPermission(session, PERMISSIONS.accountingPayablesManage) ? (
        <VendorMatchForm
          endpoint={`/api/accounting/payables/bills/${id}/matching`}
          currentStatus={String(
            data.match?.status || bill.matching_status || "not_required",
          )}
          billTotal={String(bill.grand_total)}
          canOverride={hasPermission(
            session,
            PERMISSIONS.accountingJournalApprove,
          )}
        />
      ) : null}
          ...
          <div className="accounting-list">
            {data.schedules.map((row) => (
              <div key={String(row.id)}>
                ...
                <b>
                  {String(bill.currency_code)} {String(row.outstanding_amount)}
                </b>
              </div>
            ))}
          </div>
          ...
          <div className="accounting-list">
            {data.creditAllocations.map((row) => (
              <div key={String(row.id)}>
                ...
                <b>
                  {String(bill.currency_code)} {String(row.allocated_amount)}
                </b>
              </div>
            ))}
      ...
      <section className="panel">
        <p className="eyebrow">Payables timeline</p>
        <h2>Accounting, matching, exception and payment history</h2>
        <div className="accounting-list">
          {timeline.map((row) => (
          ...
          {!timeline.length ? <p>No payables events recorded.</p> : null}
        </div>
      </section>
    </>
  );
