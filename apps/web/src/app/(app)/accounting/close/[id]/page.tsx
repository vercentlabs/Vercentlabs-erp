...
export default async function CloseRunPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <>
    ...
    <div className="accounting-action-bar">{canManage && !["completed", "cancelled"].includes(String(detail.run.status)) ? <AccountingActionButton endpoint={`/api/accounting/close/${id}/actions`} action="complete" label={String(detail.run.close_type) === "year" ? "Post year-end close and lock" : "Complete close"} tone="primary" /> : null}</div>
    ...
    <section className="panel"><p className="eyebrow">Evidence checklist</p><h2>Close tasks</h2><div className="accounting-list">{detail.tasks.map((row) => <div key={String(row.id)}><span><strong>{String(row.sequence)} · {String(row.name)}</strong><small>{String(row.note || "Evidence or reviewer note has not been recorded.")}</small></span><span><b>{String(row.status)}</b>{canManage ? <CloseTaskActions endpoint={`/api/accounting/close/${id}/tasks/${String(row.id)}`} status={String(row.status)} /> : null}</span></div>)}</div></section>
  </>;
}
