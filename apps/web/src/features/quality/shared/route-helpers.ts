...
import { requireQualityAccess, qualityContext } from "@/features/quality/shared/quality-context";
...
// Every Quality route: workspace session -> module entitlement + permission (+ billing write access
// for a mutation) -> ONE tenant transaction -> domain function -> uniform error mapping.
export async function qualityRead<T>(run: (client: Client, context: Context, session: WorkspaceSessionContext) => Promise<T>, permission: string = "quality.view") {
  try {
    const session = await requireWorkspace();
    const result = await tenantTransaction(session.organizationId, async (client) => {
      await requireQualityAccess(client, session, permission);
      return run(client as Client, qualityContext(session), session);
    });
...
export async function qualityMutation<I, T>(
  ...
  try {
    ...
    const result = await tenantTransaction(session.organizationId, async (client) => {
      await requireQualityAccess(client, session, permission, { mutation: true });
      return run(client as Client, qualityContext(session), input, session);
    });
