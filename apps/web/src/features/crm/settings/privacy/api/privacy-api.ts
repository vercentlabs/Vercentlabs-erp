...
// F002 Stage A2 §13. Reuses the shared platform privacy authority
// (services/api/src/core/privacy.js) via /api/privacy/* — never a
// CRM-local privacy resource.
...
export async function listRetentionPolicies(): Promise<{ rows: RetentionPolicy[] }> {
  const response = await fetch("/api/privacy/retention-policies");
  return parseResponse(response);
}
export async function writeRetentionPolicy(input: { dataClass: string; retentionDays: number; legalBasis: string; effectiveFrom?: string; effectiveTo?: string }): Promise<{ record: RetentionPolicy }> {
