...
async function parse<T>(response: Response): Promise<T> {
  ...
  if (!response.ok || payload.ok === false) throw new SupportApiError(payload.message || "The request could not be completed.", response.status, payload.code);
  return payload;
}
...
export type SupportOptions = Partial<Record<OptionKey, Array<{ id: string; code: string; name: string }>>>;
type OptionKey = import("@/features/support/shared/FieldInput").OptionSource;

export function useSupportOptions() {
  const workspace = useWorkspaceContext();
  return useQuery({ queryKey: scopedQueryKey(workspace, "support", "options"), queryFn: () => readView<{ options: SupportOptions }>("options").then((r) => r.options), staleTime: 15_000 });
}
