...
import { Button, ErrorState, IconButton, PageHeader } from "@vercentlabs/design-system";
...
function FormBody({ config, id, existing, options }: { config: FormConfig; id?: string; existing: ProcRecord | null; options: ProcOptions }) {
  ...
  const [values, setValues] = useState<Record<string, FieldValue>>(() => {
    ...
    for (const field of config.fields) {
      ...
      initial[field.name] = fromRecord !== undefined && fromRecord !== null ? (typeof fromRecord === "number" ? fromRecord : String(fromRecord).slice(0, field.kind === "date" ? 10 : undefined)) : (config.initial?.[field.name] ?? field.defaultValue ?? (field.kind === "number" ? 0 : ""));
    }
    return initial;
  });
  ...
  return (
    <div className="flex flex-col gap-6">
      <PageHeader
        ...
        description={editing ? "Only draft or rejected documents can be edited." : "Totals and the document number are set by the server when you save."}
        secondaryActions={
        ...
        primaryAction={
          <Button variant="primary" onPress={() => save.mutate()} isLoading={save.isPending}>
            {editing ? "Save changes" : `Save ${config.noun}`}
          </Button>
        }
      />
