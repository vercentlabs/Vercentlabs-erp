// PERMISSIONS, split out of authorization.ts so it can be imported by code
// that must never pull in @/lib/auth (and, transitively, @/lib/db -> pg ->
// node:async_hooks) — most importantly Client Components. This file's only
// dependency is the pure-data @vercentlabs/permissions package; it does not
// import @/lib/auth, @/lib/db, or anything Node-server-only, so it is safe
// to import from browser-bundled code. authorization.ts re-exports this
// unchanged, so none of its existing ~200 call sites needed to change.
// See docs/implementation/ERP_NAVIGATION_FOUNDATION_006.md's adversarial
// review for how this was discovered (a `next build` client-bundle failure
// when apps/web/src/core/navigation/breadcrumb-labels.ts, imported by the
// client component breadcrumbs.tsx, transitively required PERMISSIONS).
import {
...
export const PERMISSIONS = {
  ...
  crmAccountsViewSensitive: CRM_PERMISSIONS.accountsViewSensitive,
  ...
  procurementSuppliersSensitive: PROCUREMENT_PERMISSIONS.suppliersSensitive,
