...
const baseMigration = fs.readFileSync(path.resolve(process.cwd(), "../../database/tenant/migrations/012_procurement_module.sql"), "utf8");
const completionMigration = fs.readFileSync(path.resolve(process.cwd(), "../../database/tenant/migrations/013_procurement_enterprise_completion.sql"), "utf8");
const integrityMigration = fs.readFileSync(path.resolve(process.cwd(), "../../database/tenant/migrations/014_procurement_integrity_hardening.sql"), "utf8");
...
if (requiredTables.length < 31) throw new Error("Procurement table contract is incomplete.");
...
try {
  const requiredMigrations = ["012_procurement_module.sql", "013_procurement_enterprise_completion.sql", "014_procurement_integrity_hardening.sql"];
  const applied = await pool.query("SELECT name,state FROM tenant_schema_migrations WHERE name=ANY($1::text[])", [requiredMigrations]);
  const appliedSet = new Map(applied.rows.map((row) => [row.name, row.state]));
  for (const name of requiredMigrations) {
    if (appliedSet.get(name) !== "applied") throw new Error(`Procurement tenant migration ${name} is not applied.`);
  }
  ...
  `);
  if (integrityColumns.rowCount !== 7) throw new Error("Procurement normalized relationship/version columns are incomplete.");
  ...
  if (integrityIndexes.rowCount !== 2) throw new Error("Procurement idempotency/duplicate indexes are incomplete.");
  ...
  `);
  if (!awardUniqueness.rows[0]) throw new Error("Procurement sourcing-award uniqueness is missing.");
