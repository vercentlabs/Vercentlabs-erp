...
const futureRoleSlugs = [
  "inventory_manager",
  "manufacturing_manager",
  "hr_manager",
];

test("four-module role catalogue is complete and module-aware", () => {
  assert.equal(templates.length, 29);
  assert.equal(
    new Set(templates.map((role) => role.slug)).size,
    templates.length,
  );
  for (const slug of currentRoleSlugs) {
    assert.ok(
      templateBySlug.has(slug),
      `Missing current role template ${slug}`,
    );
  }
  for (const slug of futureRoleSlugs) {
    const role = templateBySlug.get(slug);
    assert.ok(role, `Missing future role template ${slug}`);
    assert.equal(role.assignable, false, `${slug} must remain unassignable`);
  }
