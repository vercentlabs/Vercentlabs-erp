...
export function CreateRecordSheet({ resource }: { resource: Resource }) {
  ...
  const [open, setOpen] = useState(false); const [values, setValues] = useState<Record<string, string>>({}); const [message, setMessage] = useState(""); const [busy, setBusy] = useState(false);
  const queryClient = useQueryClient(); const { colors, radii, spacing, type } = useTheme();
  async function submit() {
    const missing = definition.fields.find(([key, , required]) => required && !values[key]?.trim());
    if (missing) { setMessage(`${missing[1]} is required.`); return; }
    setBusy(true); setMessage(""); const idempotencyKey = Crypto.randomUUID();
    ...
    try { await mobileApi.createCrm(resource, payload, idempotencyKey); await queryClient.invalidateQueries({ queryKey: ["mobile-crm"] }); setOpen(false); setValues({}); }
    catch (error) { if ((error as { retryable?: boolean }).retryable !== false) { await enqueueMutation({ id: Crypto.randomUUID(), operation: "create", resource, payload, idempotencyKey }); setMessage(`Saved offline. This ${definition.label} will sync automatically.`); } else setMessage(error instanceof Error ? error.message : "Could not save the record."); }
