...
function RecordCard({ row, fields }: { row: Row; fields: string[] }) {
  ...
  return (
    <Panel>
      ...
      {fields
        .filter((key) => row[key] !== undefined && key !== "status")
        .map((key) => (
          <View key={key} style={{ flexDirection: "row", gap: spacing.sm }}>
            <Text
              style={{ ...type.caption, width: 106, color: colors.textMuted }}
            >
              {words(key)}
            </Text>
            <Text
              style={{ ...type.caption, flex: 1, color: colors.textSecondary }}
            >
              {display(row[key])}
            </Text>
          </View>
...
function Metric({
  ...
  return (
    ...
    >
      ...
      <Text style={{ ...type.heading, fontSize: 29, color: colors.text }}>
        {display(value)}
      </Text>
      <Text style={{ ...type.caption, color: colors.textMuted }}>{label}</Text>
      {description ? (
        <Text
          style={{ ...type.caption, fontSize: 11, color: colors.textMuted }}
        >
          {description}
        </Text>
      ) : null}
...
export function WorkspacePage({ area }: { area: string }) {
  ...
  const action = useMutation({
    mutationFn: async (input: {
      ...
      mobileApi.request<{ message?: string }>(
        input.path || `/workspace/${area}`,
        {
          method: input.method,
  ...
  const content = (() => {
    ...
    if (area === "dashboard") {
      ...
      return (
        <View style={{ gap: spacing.xl }}>
          ...
          >
            <Text style={{ color: "#D0D5DD", fontSize: 12 }}>
            ...
            <Text
              style={{ ...type.display, fontSize: 36, color: colors.inverse }}
            >
              Welcome back, {auth.session?.user.fullName.split(" ")[0]}.
            </Text>
            <Text style={{ ...type.body, color: "#D0D5DD" }}>
            ...
            >
              <Button
                label="Open modules"
                onPress={() => router.push("/(protected)/workspace/modules")}
              />
              <Button
                label="Workspace settings"
                variant="secondary"
                onPress={() => router.push("/(protected)/workspace/settings")}
              />
            </View>
          </View>
          <Panel>
          ...
          >
            ...
            {permissionSet.has("department.manage") ? (
              <Metric
                label="Departments"
          ...
          {quickActions.some((item) => permissionSet.has(item[4])) ? (
            <View style={{ gap: spacing.sm }}>
              ...
              {quickActions
                .filter((item) => permissionSet.has(item[4]))
          ...
          <View style={{ gap: spacing.sm }}>
            ...
            {rowsFor("activities").map((row) => (
              <RecordCard key={String(row.id)} row={row} fields={["due_at"]} />
            ))}
            {!rowsFor("activities").length ? (
              <Panel>
                <Text style={{ ...type.label, color: colors.text }}>
                  No open activities
                </Text>
                <Text style={{ ...type.body, color: colors.textMuted }}>
                  Assigned ERP tasks will appear here.
                </Text>
              </Panel>
            ) : null}
          </View>
          {rowsFor("recentEvents").length ? (
            <View style={{ gap: spacing.sm }}>
              ...
              {rowsFor("recentEvents").map((row, index) => (
                <RecordCard
                  key={String(row.id || index)}
                  row={row}
                  fields={["actor_name", "entity_type", "created_at"]}
                />
              ))}
            </View>
          ) : null}
    ...
    if (area === "procurement") {
      ...
      const metrics: [string, unknown, keyof typeof Ionicons.glyphMap][] = [
        [
          ...
          dashboard.pending_requisitions,
      ...
      return (
        <View style={{ gap: spacing.xl }}>
          ...
          <Panel>
            ...
            <Text style={{ ...type.body, color: colors.textMuted }}>
              Mobile provides a secure operational summary. Complex sourcing
              comparisons, contract amendments and bulk line editing remain
              available in the responsive web workspace.
            </Text>
          </Panel>
        </View>
      );
    ...
    if (area === "crm") {
      ...
      const metricItems: [string, unknown, string][] = [
        ...
        ["Conversions this month", metrics.conversionsThisMonth, "conversion"],
      ];
      return (
        <View style={{ gap: spacing.xl }}>
          <View
          ...
          >
            <Button
              ...
              onPress={() =>
                router.push({
                  ...
                  params: { area: "crm", resource: "leads", create: "1" },
                })
              }
            />
            <Button
            ...
            <Button
              ...
              onPress={() =>
                router.push({
                  pathname: "/(protected)/workspace/[area]/[resource]",
                  params: { area: "crm", resource: "activities", create: "1" },
                })
              }
            />
            <Button
            ...
            <Button
              label="CRM settings"
              variant="secondary"
              onPress={() => router.push("/(protected)/workspace/crm-settings")}
            />
          </View>
          <View
            style={{ flexDirection: "row", flexWrap: "wrap", gap: spacing.sm }}
          >
            {metricItems.map(([label, value, hint]) => (
              <Metric
                key={label}
                label={label}
                value={value}
          ...
          <View style={{ gap: spacing.sm }}>
            ...
            {((dashboard.stages || []) as Row[]).map((row) => (
              <RecordCard
                key={String(row.id)}
                row={{
                  title: row.name,
          ...
          <View style={{ gap: spacing.sm }}>
            <View style={{ flexDirection: "row", alignItems: "center" }}>
              ...
              <Pressable
                onPress={() =>
                  router.push("/(protected)/workspace/crm-reports")
                }
              >
                <Text style={{ ...type.label, color: colors.primary }}>
                  Reports
            ...
            {((dashboard.sources || []) as Row[]).map((row, index) => (
              <RecordCard
          ...
          <View style={{ gap: spacing.sm }}>
            <View style={{ flexDirection: "row", alignItems: "center" }}>
              ...
              <Pressable
                onPress={() =>
                  router.push({
                    pathname: "/(protected)/workspace/[area]/[resource]",
                    params: { area: "crm", resource: "activities" },
                  })
                }
              >
                <Text style={{ ...type.label, color: colors.primary }}>
            ...
            {((dashboard.activities || []) as Row[]).map((row) => (
              <RecordCard
                key={String(row.id)}
                row={{ ...row, title: row.subject }}
                fields={["activityType", "assignedName", "dueAt"]}
              />
            ))}
            {!((dashboard.activities || []) as Row[]).length ? (
              <Panel>
                <Text style={{ ...type.label, color: colors.text }}>
                  No pending activities
                </Text>
                <Text style={{ ...type.body, color: colors.textMuted }}>
                  Create a task, call, meeting or follow-up for the next
                  customer action.
                </Text>
                <Button
                  label="Create activity"
      ...
      return (
        <View style={{ gap: spacing.md }}>
            ...
            >
              ...
              >
                <Text style={{ ...type.heading, color: colors.inverse }}>
                  {String(user?.fullName || "U").slice(0, 1)}
                </Text>
              </View>
              <View style={{ flex: 1 }}>
                <Text style={{ ...type.heading, color: colors.text }}>
          ...
          <Panel>
            ...
            {(["fullName", "locale", "timezone", "theme"] as const).map(
              (key) => (
            ...
            <Button
              ...
              onPress={() =>
                action.mutate({ method: "PATCH", body: profileInput })
              }
            />
          </Panel>
          <Text style={{ ...type.heading, color: colors.text }}>
          ...
          {rowsFor("logins").map((row, index) => (
            <RecordCard
              key={index}
              row={row}
              fields={["created_at", "succeeded", "ip_address", "reason"]}
            />
          ))}
        </View>
      );
      ...
      return (
        <View style={{ gap: spacing.md }}>
          <Panel>
              ...
              (key) => (
                <View key={key} style={{ gap: 5 }}>
                  <Text
                    style={{ ...type.caption, color: colors.textSecondary }}
                  >
                    {key === "password" ? "New password" : words(key)}
                  </Text>
                  <TextInput
                    secureTextEntry
                    value={password[key]}
            ...
            <Text style={{ ...type.caption, color: colors.textMuted }}>
              Changing your password revokes all sessions and requires sign-in
              again.
            </Text>
            <Button
              label="Change password"
              loading={action.isPending}
              onPress={() => action.mutate({ method: "POST", body: password })}
            />
          </Panel>
          <View style={{ flexDirection: "row", alignItems: "center" }}>
          ...
          {rowsFor("sessions").map((row) => (
            <View key={String(row.id)}>
              ...
              />
              {String(row.id) !== String(data.currentSessionId) ? (
                <Pressable
                  onPress={() =>
                    action.mutate({
                      method: "PATCH",
                      body: { action: "revoke", sessionId: row.id },
                    })
                  }
                  style={{
                    alignSelf: "flex-end",
              ...
              {row.status === "pending" && row.command_key ? (
                <Panel>
                  <TextInput
                    ...
                    placeholderTextColor={colors.textMuted}
                    value={decisionNotes[String(row.id)] || ""}
                    onChangeText={(value) =>
                  ...
                  <View style={{ flexDirection: "row", gap: spacing.sm }}>
                    <Button
                      ...
                      onPress={() =>
                        action.mutate({
                          path: `/approvals/${String(row.id)}`,
                          method: "PATCH",
                          body: {
                            action: "approve",
                            note: decisionNotes[String(row.id)] || "",
    ...
    if (area === "crm-reports") {
      ...
      return (
        <View style={{ gap: spacing.xl }}>
          {(Array.isArray(data.names) ? data.names.map(String) : []).map(
            (name) => {
        ...
        <View style={{ gap: spacing.md }}>
          ...
          {rowsFor("modules").map((module, index) => {
            ...
            const status = released
              ? statuses.get(String(module.key)) || "enabled"
              : "roadmap";
            return (
  ...
  return (
    <Screen>
      <AppHeader
        eyebrow={page?.eyebrow || "Workspace"}
        title={page?.title || words(area)}
