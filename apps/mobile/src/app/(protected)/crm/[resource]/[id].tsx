...
export default function RecordDetailScreen() {
  ...
  const stagesQuery = useCrmQuery("pipeline-stages:detail", () => mobileApi.listCrm("stages", { limit: 100 }));
  ...
  async function convertLead() { setBusy(true); setMessage(""); try { const result = await mobileApi.apiRequest<{ message?: string }>(`/crm/leads/${params.id}/convert`, { method: "POST", body: JSON.stringify({ createOpportunity: true }) }); await query.refetch(); setMessage(result.message || "Lead converted."); } catch (error) { setMessage(error instanceof Error ? error.message : "Could not convert lead."); } finally { setBusy(false); } }
  async function mergeLead(targetLeadId: string) { setBusy(true); setMessage(""); try { const result = await mobileApi.apiRequest<{ message?: string }>(`/crm/leads/${params.id}/merge`, { method: "POST", body: JSON.stringify({ targetLeadId }) }); setMessage(result.message || "Lead merged."); router.replace({ pathname: "/(protected)/crm/[resource]/[id]", params: { resource: "leads", id: targetLeadId } }); } catch (error) { setMessage(error instanceof Error ? error.message : "Could not merge lead."); } finally { setBusy(false); } }
  return <Screen refreshControl={<RefreshControl refreshing={query.isFetching} onRefresh={() => void query.refetch()} tintColor={colors.primary} />}>
    <AppHeader eyebrow={`${resource.replace("opportunities", "Opportunity").replace("activities", "Activity").replace("leads", "Lead")}${record?.code ? ` · ${String(record.code)}` : ""}`} title={title} description={record ? String(record.nextStep ?? record.description ?? record.companyName ?? record.email ?? "CRM record detail") : undefined} />
    {record?.status ? <View style={{ alignSelf: "flex-start", marginBottom: spacing.md }}><StatusPill value={String(record.status)} /></View> : null}
    ...
    {record ? <>
      ...
      {resource === "leads" && record.status !== "converted" && record.status !== "archived" ? <Pressable disabled={busy} onPress={() => Alert.alert("Convert lead?", "Create the customer, contact and opportunity while preserving this history.", [{ text: "Cancel", style: "cancel" }, { text: "Convert", onPress: () => void convertLead() }])} style={{ minHeight: 52, marginBottom: spacing.lg, borderRadius: radii.sm, backgroundColor: colors.primary, justifyContent: "center", alignItems: "center" }}><Text style={{ ...type.label, color: colors.inverse }}>{busy ? "Working…" : "Convert lead"}</Text></Pressable> : null}
      {resource === "leads" && related.duplicates?.length ? <View style={{ marginBottom: spacing.lg, gap: spacing.sm }}><Text style={{ ...type.heading, color: colors.text }}>Potential duplicates</Text>{related.duplicates.map((duplicate) => <View key={String(duplicate.id)} style={{ padding: spacing.md, borderWidth: 1, borderColor: colors.border, borderRadius: radii.md, backgroundColor: colors.surface, gap: spacing.xs }}><Text style={{ ...type.label, color: colors.text }}>{String(duplicate.fullName ?? duplicate.code)}</Text><Text style={{ ...type.caption, color: colors.textMuted }}>{String(duplicate.companyName ?? "No company")}</Text><Pressable disabled={busy} onPress={() => Alert.alert("Merge lead?", "The current lead will be archived and its history moved to this record.", [{ text: "Cancel", style: "cancel" }, { text: "Merge", style: "destructive", onPress: () => void mergeLead(String(duplicate.id)) }])}><Text style={{ ...type.label, color: colors.danger }}>Merge into this lead</Text></Pressable></View>)}</View> : null}
      ...
      {Object.entries(related).filter(([section]) => section !== "duplicates").map(([section, rows]) => (
        <View key={section} style={{ marginTop: spacing.xl, gap: spacing.sm }}>
          <Text style={{ ...type.caption, color: colors.primary, textTransform: "uppercase" }}>{section.replace(/([A-Z])/g, " $1")}</Text>
          <Text style={{ ...type.heading, color: colors.text }}>{section.replace(/([A-Z])/g, " $1").replace(/^./, (letter) => letter.toUpperCase())}</Text>
          {rows.length ? rows.map((row, index) => {
            const heading = String(row.subject ?? row.name ?? row.item_name ?? row.reason ?? row.from_stage ?? row.channel ?? row.id ?? "Event");
            const detail = String(row.description ?? row.body ?? row.outcome ?? row.note ?? row.status ?? row.to_stage ?? "");
            const time = row.changed_at ?? row.occurred_at ?? row.completed_at ?? row.due_at ?? row.created_at;
            return <View key={String(row.id ?? index)} style={{ padding: spacing.md, borderRadius: radii.md, backgroundColor: colors.surface, borderWidth: 1, borderColor: colors.border, gap: spacing.xs }}><Text style={{ ...type.label, color: colors.text }}>{heading}</Text>{detail ? <Text style={{ ...type.body, color: colors.textSecondary }}>{detail}</Text> : null}{time ? <Text style={{ ...type.caption, color: colors.textMuted }}>{new Date(String(time)).toLocaleString()}</Text> : null}</View>;
          }) : <View style={{ padding: spacing.lg, borderRadius: radii.md, backgroundColor: colors.surface, borderWidth: 1, borderColor: colors.border }}><Text style={{ ...type.body, color: colors.textMuted }}>No {section.replace(/([A-Z])/g, " $1").toLowerCase()} recorded.</Text></View>}
        </View>
      ))}
