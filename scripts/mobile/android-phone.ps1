...
if ($SkipBuild) {
  Write-Warning "-SkipBuild is deprecated because runtime-only launch is now the default."
}
if ($Rebuild -and ($repoRoot -match "\s" -or $repoRoot.Length -gt 12)) {
  throw "-Rebuild requires a short, space-free clone. Run it from C:\ve (or another short clone)."
}
...
$adb = Join-Path $physicalSdk "platform-tools\adb.exe"
...
try {
  ...
  if ($Rebuild) {
    Write-Host "Android app rebuilt, installed and launched on $Serial."
    Write-Host "APK: $apk"
  } else {
    Write-Host "Installed Android app launched on $Serial from $repoRoot."
  }
