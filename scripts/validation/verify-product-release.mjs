...
for (const marker of [
  ...
  if (!controlMigration.includes(marker)) {
    failures.push(`Released product billing scope is missing: ${marker}`);
  }
}
...
for (const file of [
  "apps/web/src/app/api/accounting/journals/route.ts",
  "apps/web/src/app/api/accounting/receivables/invoices/route.ts",
  "apps/web/src/app/api/accounting/payables/bills/route.ts",
  "apps/web/src/app/api/accounting/banking/statements/route.ts",
]) {
  requireMarkers(file, ["tenantTransaction", "client"]);
}
...
if (
  ...
  failures.push(
    "The public hero presents an incorrect released module count.",
  );
}
...
if (failures.length) {
  console.error("Product release verification failed:");
  for (const failure of failures) console.error(`- ${failure}`);
  process.exit(1);
}

console.log(
  `Product release scope verified: ${expectedReleasedModules.length} released modules, ${roadmapModuleKeys.length} roadmap modules and production controls.`,
);
