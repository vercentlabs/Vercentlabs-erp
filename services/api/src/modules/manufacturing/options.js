...
export async function listManufacturingOptions(client, c) {
  ...
  const [items, warehouses, uoms, boms, routings, workCenters, workOrders] = await Promise.all([
  ...
  return { items: items.rows, warehouses: warehouses.rows, uoms: uoms.rows, boms: boms.rows, routings: routings.rows, workCenters: workCenters.rows, workOrders: workOrders.rows };
}
