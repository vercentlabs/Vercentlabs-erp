// Real PostgreSQL integration test -- Sales order lifecycle: creation, approval
// gate, confirmation with the credit-limit exposure check and finance override
// (F042-F045), holds (F047), versioned amendments with approval (F048),
// fulfilment / invoice hand-off idempotency (F049-F050) and cancellation.
// Runs against the real domain layer and real RLS; nothing is mocked.
...
test("Sales order lifecycle against real PostgreSQL", async (t) => {
  ...
  try {
    ...
    let confirmed;
    await t.test("F044: submit -> (approve) -> confirm; the confirmation is a state change with an audit event", async () => {
    ...
    await t.test("F044: an unconfirmed (draft) order cannot skip the approval gate by calling approve as its author", async () => {
      const order = await tx((c) => createSalesOrder(c, sellerContext, document()));
    ...
    await t.test("F045: credit limit exceeded blocks confirmation; only a finance override with a reason passes", async () => {
    ...
    await t.test("F047: a hold needs a reason, blocks the order, and releasing it restores confirmed", async () => {
    ...
    await t.test("F048: amending a confirmed order needs a reason, creates version 2 in approval, and only a different approver applies it", async () => {
    ...
    await t.test("F051: advance payments are capped at the order total and need a reference", async () => {
    ...
    await t.test("F052: credit adjustments need a valid type, a reason, and cannot exceed the order", async () => {
    ...
    await t.test("F054: a drop-ship request is bounded by the line quantity and idempotent on its key", async () => {
    ...
    await t.test("F053: returns are limited to fulfilled quantity, idempotent, and listed in the register", async () => {
      const line = (await admin.query(`SELECT line.id FROM tenant.sales_order_lines line JOIN tenant.sales_orders o ON o.current_version_id=line.sales_order_version_id WHERE o.organization_id=$1 AND o.id=$2`, [orgId, confirmed.id])).rows[0];
